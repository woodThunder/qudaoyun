define(['text!./busitemplate.html', 'ocm_common', 'handlebars', 'ocm_baseview',
'text!./billTpl.html', 'text!./pclistTpl.html',
'./meta.js'],
    function (tpl, common, Handlebars, baseview, billTpl, pclistTpl) {
        'use strict'
        ...
        events = {
            ...
            showBillEditor: function(templateSchema) {
              if (!templateSchema.tabs) {
                templateSchema.tabs = [];
                viewModel.hasChildTabs(false);
              } else {
                viewModel.hasChildTabs(true);
              }
              createDt();
              showTplEditor(templateSchema);
              bindEvents();
            ...
            //显示字段选择器
            showFieldSelector: function(objectKey, itemsDtId, isParent) {
              ...
              viewModel.searchFieldByObject(busiObject, objectKey, function afterSearch() {
                ...
                viewModel.FieldSelector.setSimpleData(filteredProperties, {unSelect: true});
            ...
            addField: function() {
              ...
              if (selectedFields && selectedFields.length > 0) {
                selectedFields.map(function(item){
                  item.order = item.order ? item.order : "0"
                })
              }
            ...
            addExtfield: function() {
              ...
              var busiObject = viewModel.BusiTemplateParent.getValue("busiObject");
              var objectkey = viewModel.currentFieldSelector.objectKey;
                ...
                // var data = {success: "1"}
                  if(data.success == "1") { //启用扩展字段成功
                    viewModel.searchFieldByObject(busiObject, objectkey, function afterSearch() {
                      var allProperties = viewModel.MetaObjectProperties.properties;
                      var existProperties = [];
                      if (viewModel.currentFieldSelector.isParent) {
            ...
            showRefrel: function(focusRow) {
              ...
              if(type === "refer") {
                ...
                $._ajax({
                  type: 'get',
                  url: '/occ-base/base/ref-docs/getReturnFields',
                  data: {
                    id: refkey
                  },
                  success: function(data) {
                    ...
                    viewModel.AllRefrel.setSimpleData(datas, {unSelect: true});
                    $('.ui-template-property-refrel').show();
                    //参照显示内容只能是"code"或"name"
                    var couldRefshowcontent = [];
                    datas.forEach(function(refrelObj) {
        ...
        function afterRender() {
            //启用扩展字段
            $('#fieldSelectorDialog').on('click', '.ui-template-addextfield', function() {
              viewModel.extfieldDialog = u.dialog({
                content: "#extfieldDialog",
                width: "400px",
              });
              var curFieldData = viewModel.FieldSelector.getCurrentRow().getSimpleData();
              curFieldData.enable = "1";
              viewModel.Extfield.removeAllRows();
              var curRow = viewModel.Extfield.createEmptyRow();
              curRow.setSimpleData(curFieldData);
              // curRow.setValue("key", $(this).data("fieldkey"));
              // curRow.setValue("dr", 0);
              // curRow.setValue("id", $(this).data("fieldid") +'');
              viewModel.Extfield.setRowFocus(curRow);
              viewModel.Extfield._objectkey = $(this).data("objectkey");
            });
        ...
        function appInit(element, params) {
            ...
            $._ajax({
              type: 'get',
              url: '/fmcg-dynamic-object/base/busiobjects',
              dataType: 'json',
