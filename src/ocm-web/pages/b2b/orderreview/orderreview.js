...
define(['text!./orderreview_tpl.html', 'ocm_common', 'ocm_baseview', './meta.js'], function (tpl, common, baseview, model) {
  var viewModel;
      ...
      pass: function () {
        ...
        if (rows && rows.length > 0) {
          ...
          common.dialog.confirmDialog({
            ...
            onOk: function () {
              $._ajax({
                url: appCtx + viewModel.baseurl + '/order-financial-approve-pass?id=' + ids.join(','),
                type: "post",
                success: function (data) {
                  console.log(data)
      ...
      nopass: function () {
        ...
        if (rows && rows.length > 0) {
          ...
          common.dialog.confirmDialog({
            ...
            onOk: function () {
              $._ajax({
                url: appCtx + viewModel.baseurl + '/order-prohibiting-credit-loans?id=' + ids.join(','),
                type: "post",
                success: function (data) {
                  console.log(data)
                  if (data.success == 'success') {
