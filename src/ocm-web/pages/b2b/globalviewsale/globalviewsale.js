...
~ function () {
	...
	window.myChart2 = echarts.init(document.getElementById('sales-money'));
	var option2 = {
		"color": ["#FC6C24", "#21BDFC"],
		"grid": {
			"top": 50,
...
$(function () {
	...
	$('.settingBtn').on('click', 'a', function () {
		...
		if (_index == 1) {
			var data = (new Date($('#startDate').val()).getTime()) || new Date().getTime();
			var timeVal = $.trim($('#timerQuery').val());
			OrderSummar = [];
			OrderSummarDate = [];
			var time = parseFloat(timeVal) * 60 * 1000;
			queryData = {
				beginDate: data,
				saleOrg: ($('#saleOrg').attr('refid')) || ' '
			}
			searchAjax(queryData);
		...
		success: function (resp) {
			if (resp && resp.length > 0) {
				var html = resp.map(function (item) {
					...
					return '<li><span>' + item.creator + '</span><span class="color-golden">' + item.totalDealAmount + '</span> <span>' + date + '</span></li>'
				});
				$('.realTime-value').html('').append(html);
			}
		}
	})
};
...
function searchSalemanager(data) {
	...
	$.ajax({
		...
		success: function (resp) {
			...
			$('.sales-person ul').html('').append(html);
		}
	})
};

// 查询时间段内销售额
function searchOrderSummary(data) {
	var body = data ? data : {
		beginDate: ''
	}
	$.ajax({
		url: '/occ-b2b-order/b2b/order/find-order-summary',
		data: body,
		type: 'get',
		success: function (resp) {
			OrderSummar.push(parseFloat(resp.value));
			OrderSummarDate.push(timestampToTime(resp.time));
			// for (var i = 0; i < 20; i++) {
			// 	OrderSummar.push(parseFloat(resp.value + i * 100));
			// 	OrderSummarDate.push(timestampToTime(resp.time));
			// }
...
function searchSaleSummary(data) {
	...
	$.ajax({
		...
		success: function (resp) {
			if (resp) {
				$('#totalSales').html(resp.totalDealAmount);
				$('#totalMainNum').html(resp.totalNum);
				$('#saleMainNum').html(resp.totalTransVol);
			}
		}
	})
};
...
function searchProvSaleSummary(data) {
	...
	$.ajax({
		url: '/occ-b2b-order/b2b/order/find-prov-summary',
		data: body,
		type: 'get',
		success: function (resp) {
...
function ref (el, data) {
	...
	var html = '<div class="select-container"><ul>';
	data.content.forEach(function (item) {
		html += '<li refId="'+ item.id +'">'+ item.name +'</li>'
	});
	html += '</ul></div>';
	$el.parent().find('.select-container').remove();
	...
	var _container = $el.parent().find('.select-container ul');
	if (_container) {
		_container.on('click', 'li', function(){
			var _index = $(this).index;
			var _id = $(this).attr('refid'),
				_val = $(this).html(),
				prev = $(this).parents('.select-container').prev();
