...
define(['text!./headquartersapproval_tpl.html', 'ocm_common', 'ocm_baseview', './meta.js'], function (tpl, common, baseview, model) {
  var viewModel;
  var view = baseview.extend({
    ...
    events: {
      ...
      search: function (reindex) {
        ...
        if (viewModel.searchcomp.getSearchData().orderStatus) {
          ...
          queryData["search_IN_orderStatus"] = "Ext04";
        }
      ...
      pass: function () {
        ...
        if (rows && rows.length > 0) {
          ...
          common.dialog.confirmDialog({
            ...
            onOk: function () {
              $._ajax({
                url: appCtx + viewModel.baseurl + '/headquarters-approval-order?id=' + ids.join(','),
                type: "post",
                success: function (data) {
                  console.log(data)
      ...
      remarkSave: function(){
        ...
        if (basePass.passed) {
          ...
          $._ajax({
            url: appCtx + viewModel.baseurl + '/headquarters-rejected-order?id=' + viewModel.ids.join(',') + '&remark=' + dialogRemarkData.remark,
            type: "post",
            success: function (data) {
              // console.log(data)
              if (data.success == 'success') {
    ...
    afterCreate: function () {
      ...
      viewModel.searchcomp.viewModel.params.on("orderStatus.valuechange", function (obj) {
        if (obj.newValue != "Ext04") {
          $(".headquartersapproval_btn").hide();
        } else {
          $(".headquartersapproval_btn").show();
        }
      })
    }
  });
