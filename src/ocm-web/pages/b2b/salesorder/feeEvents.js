		...
		return {
				...
				$._ajax({
					...
					success: function (data) {
						if (data && data.length > 0) {
							...
							data.forEach(function (item) {
								// 记录原itemId记录原itemId
								item.copyItemId = item.orderItemId;
								// 清空字段
								item.orderId = "";
								item.orderCode = "";
								item.orderItemId = "";
			...
			fillOrderItemsOffsetDetailOnSave: function (orderItems) {
				...
				if (offsetDetails && offsetDetails.length > 0) {
					orderItems.forEach(function (item) {
						...
						// var key = item.id ? item.id : item.goodsId + item.rowNum;
						var filterDetails = offsetDetails.filter(function (detail) {
							var tempKey = detail.goodsId + detail.rowNum;
							// var tempKey = item.id ? detail.copyItemId : detail.goodsId + detail.rowNum;
							return key == tempKey;
						});
				...
				} else if (viewModel.oneKeyOffsetListDto && viewModel.oneKeyOffsetListDto.length > 0) {
					orderItems.forEach(function (item) {
						var key = item.rowNum;
						var filterDetails = viewModel.oneKeyOffsetListDto.filter(function (detail) {
							var tempKey = detail.srcBillDetailRowno;
							return key == tempKey;
						});
						if (filterDetails && filterDetails.length > 0) {
							item.offsetDetailsDtoList = [{
								feeAccountId: filterDetails[0].accountId,
								rowNum: filterDetails[0].srcBillDetailRowno,
								totalOffsetAmount: filterDetails[0].useAmount,
								offsetType: 2,
				...
				orderItems.forEach(function (item) {
					if (item.goodsSupplement && item.goodsSupplement == 1) {
						...
						if (item.offsetDetailsDtoList && item.offsetDetailsDtoList[0].feeAccountId) offsetDetails[0].feeAccountId = item.offsetDetailsDtoList[0].feeAccountId;
			...
			oneKeyOffset: function () {
				...
				var queryData = {
					// saleOrgId: saleOrgId,
					// customerId: customerId,
					orderItems: orderEnable
				};
				$._ajax({
					type: "post",
					url: window.pathMap.b2b + "/b2b/order/use-fee?saleOrgId=" + saleOrgId + "&customerId=" + customerId,
					dataType: "json",
					data: JSON.stringify(orderEnable),
					contentType: "application/json; charset=utf-8",
					success: function (resp) {
						if (resp && resp.length > 0) {
							...
							resp.forEach(function(item) {
								var currentRow = allRows.filter(function(row) {
									return row.getValue('rowNum') == item.srcBillDetailRowno
								})
								var offSetAmount = parseFloat(item.useAmount) || 0;
								totalOffSetAmount += offSetAmount;
								if (offSetAmount > 0) {
									var curData = currentRow[0].getSimpleData();
									var dealAmount = parseFloat(curData.dealAmount),
										mainNum = parseFloat(curData.mainNum);
									var dealPrice = (dealAmount - offSetAmount) / mainNum;
									currentRow[0].setValue('dealPrice', dealPrice);
									currentRow[0].setValue("dealAmount", dealAmount - offSetAmount);
								} 
							});
							viewModel.oneKeyOffsetListDto = resp;
							viewModel.salesorderCard.setValue('offsetAmount', totalOffSetAmount);
							viewModel.offsetDetailsList.removeAllRows();
						} else {
							toastr.warning("没有查询到费用信息！")
						}
					}
				});
			},
			...
			addOffsetAccount: function (obj) {
				...
				// 在已选冲抵账户列表添加
				var offsetRow = selectedRows[0].getSimpleData();
				offsetRow.adjustOffsetMoney = offsetRow.currAccountBalance
				viewModel.offsetSelectList.addSimpleData(offsetRow, "new", { unSelect: true });
			},
