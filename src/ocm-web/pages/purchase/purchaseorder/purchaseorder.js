define(['text!./purchaseorder.html', 'ocm_common', 'ocm_baseview', './meta.js', '../../flow/bpmapproveref/bpmopenbill.js', 'ocm-citypicker'], function(tpl, common, baseview, model, bpmopenbill) {
    'use strict'
    var viewModel, app;
    var BILLPANELSTATUS = {
        ADD: "add",
    ...
    var view = baseview.extend({
        ...
        baseData: {
            ...
            status: ko.pureComputed(function() {
                ...
                enableStatus = parseInt(enableStatus);
                if (isNaN(enableStatus)) return "待处理";
                switch (enableStatus) {
                    case 1:
                        return "待处理";
                    case 2:
        ...
        rendertype: {
            approveStateRender: function(obj) {
                var showValue = "";
                var showText = parseInt(obj.value);
                if (isNaN(showText)) return "待处理";
                switch (showText) {
                    case 0:
                        showValue = "待处理";
                        break;
            ...
            beforeEditCheck: function(obj) {
                ...
                viewModel.purchaseItems.setMeta("receiveStorageId", "refparam", '{"EQ_isEnable":"1","EQ_inventoryOrg.id":"' + stockOrgId + '"}');
        ...
        events: {
            ...
            coordination: function() {
                ...
                var selId = selectedRows[0].getValue("id")
                ...
                $._ajax({
                    type: "post",
                    url: appCtx + viewModel.baseurl + "/coordination",
                    data: {
                        id: selId
                    },
            ...
            delItems: function() {
                ...
                viewModel.reCalculateTotalAmountMoneyByBom();
            },
            //保存单据
            saveBill: function() {
                var index = viewModel.index;
                //判断是否是自有
                var arrivalBelong = viewModel.purchaseItems.getValue("arrivalBelongCode");
                var receiveStorageOrg = viewModel.app.getComp("grid_complexItem").grid.getColumnByField("receiveStorageOrgId");
                if (arrivalBelong != "01") {
                    receiveStorageOrg.options.editOptions.required = false;
                } else {
                    receiveStorageOrg.options.editOptions.required = true;
                }
                //end
                ...
                purchaseBomItemsData.forEach(function(item) {
                    //之前逻辑导致删除失败，增加判断
                    if(item.persistStatus!=="fdel" && item.persistStatus!=="nrm"){
                ...
                if (_ajaxType == "post") {
                    if (complexData.state == undefined || complexData.state == null) {
                        complexData.state = 0;
                    }
                }
                $._ajax({
            ...
            /*  //保存并提交单据
              submitBill: function() {
                ...
                complexData.purchaseType = "InnerPurchase";//"内采为InnerPurchase"
                var purchaseItemsData = viewModel.purchaseItems.getSimpleData();
                var purchaseBomItemsData = viewModel.purchaseBomItems.getSimpleData();
                purchaseItemsData.forEach(function(item) {
                  item.stockStatus = "01";
                                ...
                                for (var key in data[i]) {
                                    for (var j = 0; j < rows.length; j++) {
                                        ...
                                        if (key == rows[j].getValue("goodsId")) {
                                            rows[j].setValue("unitPrice", data[i][key] ? data[i][key] : 0);
            ...
            sumPrice: function(row) {
                ...
                unitPrice = unitPrice == null ? 0 : parseFloat(unitPrice);
                var rowMoneySum = amount * unitPrice;
                row.setValue("amountMoney", rowMoneySum);
                return rowMoneySum;
            },
            //日期
            ...
            backClac: function(obj, field) {
                ...
                console.log(field + ', ' + oneParentBomSum)
            ...
            reCalculateTotalAmountMoneyByBom: function () {
        ...
        afterCreate: function() {
            ...
            viewModel.ItemRefList.on("productref.valuechange", function(obj) {
                ...
                if (refValues && refValues.length > 0) {
                    for (var i = 0; i < refValues.length; i++) {
                        ...
                        newrow.setValue("displayName", refValues[i].refname);
                        newrow.setValue("version", refValues[i].version);
                        //bom产品信息的添加
                        var bomdata = viewModel.findBomByParentId(id);
                        if (bomdata.length > 0) {
            ...
            viewModel.purchaseItems.on("goodsNum.valuechange", function(obj) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    var amountMoney = parseFloat(arr[i].amountMoney)
                    sumMoney += amountMoney;
                    // price.push(amountMoney);
                }
                viewModel.purchaseList.getCurrentRow().setValue("totalAmountMoney", sumMoney);
                // for (var i = 0; i < arr.length; i++) {
                //     if (!arr[i].goodsNum) {
                //         arr[i].goodsNum = 0
                //     }
