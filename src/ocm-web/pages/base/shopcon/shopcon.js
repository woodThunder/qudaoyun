define(['text!./shopcon.html','ocm_common','searchbox','editcard','./meta.js','ocm_global'], function (tpl,common,searchbox,editcard) {
  'use strict'
  var app,baseData,events,rendertype,viewModel,singledocSearch,singledoceidt;
  baseData = {
    baseurl : '/shop-control/getList',
    counturl : '/shop-control/getCount',
    businessTypeCode:"",
    ShopConList: new u.DataTable(ShopConmeta)
  };
  rendertype = {
  ...
  events = {
      ...
      search: function (reindex) {
        if(reindex){
          viewModel.ShopConList.pageIndex(0);
        }
        viewModel.ShopConList.removeAllRows();
        ...
        // queryData.size = viewModel.ShopConList.pageSize();
        // queryData.page = viewModel.ShopConList.pageIndex();
        $._ajax({
          type:"get",
          ...
          success:function(data){
            viewModel.ShopConList.setSimpleData(data,{unSelect:true});
            viewModel.ShopConList.totalRow(data.totalElements);
            viewModel.ShopConList.totalPages(data.totalPages);
          }
        });
        $._ajax({
          type:"get",
      ...
      //   viewModel.ShopConList.pageIndex(index);
      //   viewModel.search();
      // },
      // //页码改变时的回调函数
      // sizeChange: function (size) {
      //   viewModel.ShopConList.pageSize(size);
      //   viewModel.search();
      // },
      //导出
      exportHandle: function() {
          ...
          var templateUrl = '/shop-control-excel/downloadExcelTemplate'; //导出模板地址参数
          var excelDataUrl =  '/shop-control-excel/excelDataExport'; //导出数据地址参数
          var listData = viewModel.ShopConList; //需要导出表格的dataTable
  ...
  function afterRender(){
    //绑定输入框enter事件
    $('#ShopCon-searchcontent input').off("keydown").on("keydown",function(e){
      if (e.keyCode == 13) {
        $(this).blur();
        viewModel.search();
      }
