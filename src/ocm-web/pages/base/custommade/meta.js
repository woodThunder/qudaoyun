...
window.finishedProductSuite = {
    ...
    meta: {
	    ...
	    mainColorDocCode:{type: 'string'},//主色编码
	    ...
	    productGropCode:{type: 'string'},//产品组编码
	    ...
	    baseUnitCode:{type: 'string'},//基本单位编码
...
//	    exteriorSize:{type: 'string',required:true},//外观尺寸
	    whetherUpgrade:{type: 'integer'},//是否升级款
	    upgradeDesc:{type: 'string'},//升级款描述
	    whetherPromotion:{type: 'integer'},//是否促销款
	    proTypeCode:{type: 'string'},//物料类型
	    proTypeName:{type: 'string'},//物料类型
	    materialGroupId:{
	        type: 'string',
	        required:true,
	    ...
	    packageNum:{type: 'integer'},//包装数量
	    proExplain:{type: 'string'},//要货说明
	    madeDays:{type: 'integer',required:true},//自制天数
	    productFunctionID:{
	    	type: 'string',
    ...
    //销售产品开始
      ...
      trademarkId: {type: 'string',required:true},//商标id(销售产品)
      ...
      whetherSupply: {type: 'integer',required:true},//供货状态(销售产品)
...
//     whetherPackSuite: {type: 'integer'},//是否单包成套(包件)
       ...
       labelStandardId: {
        ...
        'refcfg':'{"ctx":"/uitemplate_web","refCode":"QY011","refName":"标签规格"}'
       },//标签规格(包件)
       ...
       cabinetArea: {type: 'float',precision:5},//柜体面积(包件)
       slidingArea: {type: 'float',precision:5},//移门面积(包件)
       sealEdgeMeters: {type: 'float',precision:5},//封边米数(包件)
       plasticUptakeMeters: {type: 'float',precision:5},//吸塑面积(包件)
       backMeters: {type: 'float',precision:5},//背板面积(包件)
       plateMeters9mm: {type: 'float',precision:5},//9MM板件面积(包件)
       plateMeters18mm: {type: 'float',precision:5},//18MM板件面积(包件)
       plateMeters25mm: {type: 'float',precision:5},//25MM板件面积(包件)
...
window.ProdInfoAndPhotoInfometa ={
    ...
    meta: {
      ...
      whetherMainPhoto: {type: 'integer'},//是否主图
    },
    pageSize: 10,
    //是否启用前端缓存
    // pageCache: true
