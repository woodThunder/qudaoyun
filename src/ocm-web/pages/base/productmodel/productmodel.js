define(['text!./productmodel.html','ocm_common','searchbox','editcard','./meta.js','ocm_global'], function (tpl,common,searchbox,editcard) {
  'use strict'
  var app,baseData,events,rendertype,viewModel,singledocSearch,singledoceidt,exportDialog,importDialog;
  baseData = {
    baseurl : '/product-model',
    productModelList: new u.DataTable(productModelmeta)
  };
  rendertype = {
  ...
  events = {
      ...
      beforeEdit: function(index,rowId) {
        ...
        if (index >= 0) {
          //修改操作
          title = "编辑";
          var currentData = viewModel.productModelList.getRowByRowId(rowId).getSimpleData();
        ...
        if(result.passed){
          ...
          $._ajax({
            ...
            success:function(data){
              ...
              if (index >= 0) {
                //获取需要修改的行
                currentRow = viewModel.productModelList.getRowByRowId(viewModel.rowId);
                //将用户填写的数据更新到productModelList上
              } else {
                //添加数据
                currentRow = viewModel.productModelList.createEmptyRow();
              }
      ...
      del: function (data,rowId) {
        if (typeof(data) == 'number') {
          viewModel.productModelList.setRowSelectbyRowId(rowId);
        }
        var ids = [];
        var rows = viewModel.productModelList.getSelectedRows();
        var status=[];
        var statustip="";
        if(rows&&rows.length>0){
          ...
          onOk: function () {
            $._ajax({
              ...
              success:function(data){
                viewModel.productModelList.removeRows(rows);
                toastr.success("删除成功");
              }
            });
      ...
      //启用
      enable: function() {
        var selectedRows = viewModel.productModelList.getSelectedRows();
        var ids = [];
        var status=[];
        var statustip="";
      ...
      //导入
  	  importHandle: function() {
  	  	var urlInfo = '/product-model-excel/excelDataImport'; //倒入地址参数
  	  	var urlStatusInfo = '/product-model-excel/excelLoadingStatus'; //请求进度地址参数
  	  ...
  	  exportHandle: function() {
	        ...
	        delete searchParams["search_LIKE_code"];
  	  	  var templateUrl = '/product-model-excel/downloadExcelTemplate'; //导出模板地址参数
  	  	  var excelDataUrl =  '/product-model-excel/excelDataExport'; //导出数据地址参数
  	  	  var listData = viewModel.productModelList; //需要导出表格的dataTable
      ...
      search: function (reindex) {
        ...
        if(reindex){
          viewModel.productModelList.pageIndex(0);
        }
        viewModel.productModelList.removeAllRows();
        var queryData = singledocSearch.getDataWithOpr();
        queryData.size = viewModel.productModelList.pageSize();
        queryData.page = viewModel.productModelList.pageIndex();
        ...
        $._ajax({
          ...
          data:queryData,
          success:function(data){
            viewModel.productModelList.setSimpleData(data.content,{unSelect:true});
            viewModel.productModelList.totalRow(data.totalElements);
            viewModel.productModelList.totalPages(data.totalPages);
          }
        })
      },
      //清空搜索条件
      ...
      pageChange: function (index) {
        viewModel.productModelList.pageIndex(index);
        viewModel.search();
      },
      //页码改变时的回调函数
      sizeChange: function (size) {
        viewModel.productModelList.pageSize(size);
  ...
  function afterRender(){
    //绑定输入框enter事件
    $('#productModel-searchcontent input').off("keydown").on("keydown",function(e){
      if (e.keyCode == 13) {
        $(this).blur();
        viewModel.search();
      }
