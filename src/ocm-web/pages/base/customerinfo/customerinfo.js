define(['text!./customerinfo.html','ocm_common','searchbox','editcard','./meta.js','ocm_global'], function (tpl,common,searchbox,editcard) {
  'use strict'
  var app,baseData,events,rendertype,viewModel,singledocSearch,singledoceidt;
  baseData = {
    baseurl : '/customer-info/getList',
    counturl : '/customer-info/getCount',
    businessTypeCode:"",
    CustomerInfoList: new u.DataTable(CustomerInfometa)
  };
  rendertype = {
  ...
  events = {
      ...
      search: function (reindex) {
        if(reindex){
          viewModel.CustomerInfoList.pageIndex(0);
        }
        viewModel.CustomerInfoList.removeAllRows();
        ...
        // queryData.size = viewModel.CustomerInfoList.pageSize();
        // queryData.page = viewModel.CustomerInfoList.pageIndex();
        $._ajax({
          type:"get",
          ...
          success:function(data){
            viewModel.CustomerInfoList.setSimpleData(data,{unSelect:true});
            viewModel.CustomerInfoList.totalRow(data.totalElements);
            viewModel.CustomerInfoList.totalPages(data.totalPages);
          }
        });
        $._ajax({
          type:"get",
      ...
      //   viewModel.CustomerInfoList.pageIndex(index);
      //   viewModel.search();
      // },
      // //页码改变时的回调函数
      // sizeChange: function (size) {
      //   viewModel.CustomerInfoList.pageSize(size);
      //   viewModel.search();
      // },
      //导出
      exportHandle: function() {
          ...
          var templateUrl = '/customer-info-excel/downloadExcelTemplate'; //导出模板地址参数
          var excelDataUrl =  '/customer-info-excel/excelDataExport'; //导出数据地址参数
          var listData = viewModel.CustomerInfoList; //需要导出表格的dataTable
  ...
  function afterRender(){
    //绑定输入框enter事件
    $('#CustomerInfo-searchcontent input').off("keydown").on("keydown",function(e){
      if (e.keyCode == 13) {
        $(this).blur();
        viewModel.search();
      }
