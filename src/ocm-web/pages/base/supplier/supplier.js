    ...
    common, baseview, model, searchbox) {
        ...
        var view = baseview.extend({
            ...
            baseData: {
				...
				excelurl: '/supplier-excel',
                statusField: 'isEnable',
                dialogWidth: '900px',
                simpleList: new u.DataTable(model.options.metas.suppliermeta),
                accountList: new u.DataTable(model.options.metas.supplieraccountmeta),
                simpleListBackup: new u.DataTable(model.options.metas.suppliermeta),
                simpleListTree: new u.DataTable(model.options.metas.suppliercategorymeta),
            ...
            events: {
                ...
                saveHandle: function () {
                    ...
                    // var baseInfo = $("#supplierBase")[0];
                    var basePass = viewModel.validate(supplierBase);
                    if (basePass.passed) {

                        // var index = viewModel.index;
                        var simpleData = viewModel.simpleList.getFocusRow().getSimpleData();
                        if (common.validStrLength(simpleData.name) > 200) {
                          toastr.warning("供应商名字超出长度限制，请调整");
                          return;
                        }
                        simpleData.supplierAccounts = viewModel.accountList.getSimpleData().length?viewModel.accountList.getSimpleData():null;
                        if(simpleData.isCarrier) {
                            if(!simpleData.carrierType) {
                                toastr.warning("请选择承运商类型");
                                return;
                            }
                        }
                        var type = "post";
            ...
            afterCreate: function () {
                ...
                // 基本信息
                viewModel.simpleList.on("supplierTypeCode.valuechange", function (
                    obj
                ) {
                    var innerOrg = viewModel.app.getComp("innerOrgId");
                    if (obj.newValue == "01") {
                        ...
                        innerOrg.setEnable(false);
                        viewModel.simpleList.setValue("innerOrgId","")
                        viewModel.simpleList.setValue("innerOrgCode","")
                        viewModel.simpleList.setValue("innerOrgName","")
                        
                    }

                });
                viewModel.simpleList.on("isCarrier.valuechange", function (obj){
                    var newValue = obj.newValue;
                    var carrierType = viewModel.app.getComp("carrierType");
                    if(newValue) {
                        carrierType.setEnable(true);
                    } else {
                        viewModel.simpleList.setValue("carrierType","");
                        viewModel.simpleList.setValue("carrierTypeName","");
                        carrierType.setEnable(false);
                    }
                });
