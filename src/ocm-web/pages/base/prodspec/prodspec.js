define(['text!./prodspec.html','ocm_common','searchbox','editcard','./meta.js','ocm_global'], function (tpl,common,searchbox,editcard) {
  'use strict'
  var app,baseData,events,rendertype,viewModel,singledocSearch,singledoceidt;
  baseData = {
    baseurl : '/prod-spec',
    prodSpecList: new u.DataTable(prodSpecmeta)
  };
  rendertype = {
  ...
  events = {
      ...
      beforeEdit: function(index,rowId) {
        ...
        if (index >= 0) {
          //修改操作
          title = "编辑";
          var currentData = viewModel.prodSpecList.getRowByRowId(rowId).getSimpleData();
        ...
        if(result.passed){
          ...
          $._ajax({
            ...
            success:function(data){
              ...
              if (index >= 0) {
                //获取需要修改的行
                currentRow = viewModel.prodSpecList.getRowByRowId(viewModel.rowId);
                //将用户填写的数据更新到prodSpecList上
              } else {
                //添加数据
                currentRow = viewModel.prodSpecList.createEmptyRow();
              }
      ...
      del: function (data,rowId) {
        if (typeof(data) == 'number') {
          viewModel.prodSpecList.setRowSelectbyRowId(rowId);
        }
        var ids = [];
        var rows = viewModel.prodSpecList.getSelectedRows();
        var status=[];
        var statustip="";
        if(rows&&rows.length>0){
          ...
          onOk: function () {
            $._ajax({
              ...
              success:function(data){
                viewModel.prodSpecList.removeRows(rows);
                toastr.success("删除成功");
              }
            });
     ...
     //启用
      enable: function() {
        var selectedRows = viewModel.prodSpecList.getSelectedRows();
        var ids = [];
        var status=[];
        var statustip="";
      ...
      search: function (reindex) {
        if(reindex){
          viewModel.prodSpecList.pageIndex(0);
        }
        viewModel.prodSpecList.removeAllRows();
        var queryData = singledocSearch.getDataWithOpr();
        queryData.size = viewModel.prodSpecList.pageSize();
        queryData.page = viewModel.prodSpecList.pageIndex();
        $._ajax({
          type:"get",
          url:appCtx + viewModel.baseurl,
          dataType:"json",
          data:queryData,
          success:function(data){
            viewModel.prodSpecList.setSimpleData(data.content,{unSelect:true});
            viewModel.prodSpecList.totalRow(data.totalElements);
            viewModel.prodSpecList.totalPages(data.totalPages);
          }
        })
      },
      //清空搜索条件
      ...
      pageChange: function (index) {
        viewModel.prodSpecList.pageIndex(index);
        viewModel.search();
      },
      //页码改变时的回调函数
      sizeChange: function (size) {
        viewModel.prodSpecList.pageSize(size);
        viewModel.search(true);
      },
      //导入
      importHandle: function() {
        var urlInfo = '/prod-spec-excel/excelDataImport'; //倒入地址参数
        var urlStatusInfo = '/prod-spec-excel/excelLoadingStatus'; //请求进度地址参数
      ...
      exportHandle: function() {
          ...
          var templateUrl = '/prod-spec-excel/downloadExcelTemplate'; //导出模板地址参数
          var excelDataUrl =  '/prod-spec-excel/excelDataExport'; //导出数据地址参数
          var listData = viewModel.prodSpecList; //需要导出表格的dataTable
  ...
  function afterRender(){
    //绑定输入框enter事件
    $('#prodSpec-searchcontent input').off("keydown").on("keydown",function(e){
      if (e.keyCode == 13) {
        $(this).blur();
        viewModel.search();
      }
