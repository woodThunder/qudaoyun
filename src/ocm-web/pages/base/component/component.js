define(['text!./component.html','ocm_common','searchbox','editcard','./meta.js','ocm_global'], function (tpl,common,searchbox,editcard) {
  'use strict'
  var app,baseData,events,rendertype,viewModel,singledocSearch,singledoceidt;
  baseData = {
    baseurl : '/components',
    componentList: new u.DataTable(componentmeta)
  };
  rendertype = {
  ...
  events = {
      ...
      beforeEdit: function(index,rowId) {
        ...
        if (index >= 0) {
          //修改操作
          title = "编辑";
          var currentData = viewModel.componentList.getRowByRowId(rowId).getSimpleData();
        ...
        if(result.passed){
          ...
          $._ajax({
            ...
            success:function(data){
              ...
              if (index >= 0) {
                //获取需要修改的行
                currentRow = viewModel.componentList.getRowByRowId(viewModel.rowId);
                //将用户填写的数据更新到componentList上
              } else {
                //添加数据
                currentRow = viewModel.componentList.createEmptyRow();
              }
      ...
      del: function (data,rowId) {
        if (typeof(data) == 'number') {
          viewModel.componentList.setRowSelectbyRowId(rowId);
        }
        var ids = [];
        var rows = viewModel.componentList.getSelectedRows();
        var status=[];
        var statustip="";
        if(rows&&rows.length>0){
	        ...
	        common.dialog.confirmDialog({
	          ...
	          onOk: function () {
	            $._ajax({
	              ...
	              success:function(data){
	                viewModel.componentList.removeRows(rows);
	                toastr.success("删除成功");	
	              }
	            });
         ...
         //启用
      enable: function() {
        var selectedRows = viewModel.componentList.getSelectedRows();
        var ids = [];
        var status=[];
        var statustip="";
      ...
      search: function (reindex) {
        if(reindex){
          viewModel.componentList.pageIndex(0);
        }
        viewModel.componentList.removeAllRows();
        var queryData = singledocSearch.getDataWithOpr();
        queryData.size = viewModel.componentList.pageSize();
        queryData.page = viewModel.componentList.pageIndex();
        $._ajax({
          type:"get",
          url:appCtx + viewModel.baseurl,
          dataType:"json",
          data:queryData,
          success:function(data){
            viewModel.componentList.setSimpleData(data.content,{unSelect:true});
            viewModel.componentList.totalRow(data.totalElements);
            viewModel.componentList.totalPages(data.totalPages);
          }
        })
      },
      //清空搜索条件
      ...
      pageChange: function (index) {
        viewModel.componentList.pageIndex(index);
        viewModel.search();
      },
      //页码改变时的回调函数
      sizeChange: function (size) {
        viewModel.componentList.pageSize(size);
      ...
      exportHandle: function() {
          ...
          var listData = viewModel.componentList; //需要导出表格的dataTable
  ...
  function afterRender(){
    //绑定输入框enter事件
    $('#component-searchcontent input').off("keydown").on("keydown",function(e){
      if (e.keyCode == 13) {
        $(this).blur();
        viewModel.search();
      }
