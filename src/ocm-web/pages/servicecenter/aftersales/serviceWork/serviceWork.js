define(['text!./serviceWork.html', 'ocm_common', 'searchbox', 'editcard', 'ocm_repairtrack',
 './meta.js', 'ocm_global'], function (tpl, common, searchbox, editcard, repairTrack) {
  'use strict'
  ...
  rendertype = {
    ...
    operation: function (obj) {
      ...
      //   sendOrderHide = ' hide';
      // }
      if (billStatus == '1' || billStatus == '2' || billStatus == '3') {
  ...
  events = {
    ...
    dispatchingFun: function (index, rowId) {
      viewModel.mainListData.setRowSelectbyRowId(rowId);
      var currentRow = viewModel.mainListData.getFocusRow().getSimpleData();
      viewModel.flag(currentRow.isOrder == "0");
      ...
      okButton.unbind("click").click(function () {
        ...
        var personId = currentRow.servicePersonId;
        if(personId){
          currentRow.billStatus = '1';
        }
        $._ajax({
       ...
       // 预约按钮
       var yueButton = $("#dispatching-popup .u-msg-yue");
       yueButton.unbind("click").click(function () {
        var myData = viewModel.mainListData.getCurrentRow().getSimpleData();
            myData.servicePersonId = null;
            myData.isOrder = '1';
        $._ajax({
          url: appCtx + viewModel.baseurl,
          type: "put",
