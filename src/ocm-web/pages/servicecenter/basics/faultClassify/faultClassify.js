define(['text!./faultClassify.html', 'ocm_common', 'searchbox', 'editcard', './meta.js', 'ocm_global'], function(tpl, common, searchbox, editcard) {
	'use strict'
	var app, baseData, events, rendertype, viewModel, singledocSearch,popupDialog,popupMainDataListData;
	baseData = {
		baseurl: '/sc/fault-types',
			...
			if (typeof (rowId) == 'number') {
			  ...
			  if (rows && rows.length > 0) {
				for (var i = 0; i < rows.length; i++) {
					...
					var code = rows[i].getValue("code")
					$._ajax({
					url: appCtx + viewModel.baseurl + "/getChildrenByParentId/"+id,
					type: "get",
					async: false,
					success: function (data) {
						if(!data){
						   var message = "分类编码:"+code+" 被下级引用不能删除！"
