define(['text!./customerEquity.html', 'ocm_common', 'searchbox', 'editcard', './meta.js', 'ocm_global'], function (tpl, common, searchbox, editcard) {
    'use strict'
    ...
    events = {
      ...
      disabledBegin: function (current) {
        var endTime = viewModel.customerequityMeta.getValue("warrantyEndDate");
        if (endTime) {
      ...
      disabledEnd: function (current) {
        var beginTime = viewModel.customerequityMeta.getValue("warrantyStartDate");
        if (beginTime) {
      ...
      beforeEdit: function (index) {
        ...
        if (index >= 0) {
          ...
          title = "新增";
          var row = viewModel.customerequityMeta.createEmptyRow();
          viewModel.customerequityMeta.setRowFocus(row);
          viewModel.goBillPanel();
