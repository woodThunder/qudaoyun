define(['text!./helpsaleapply.html', 'ocm_common', 'ocm_baseview', './meta.js', '../../flow/bpmapproveref/bpmopenbill.js', "ajaxfileupload",
  "ossupload",
  "interfaceFileImpl",
  ...
  var view = baseview.extend({
    ...
    baseData: {
      baseurl: '/gift-applys',
      applicationUrl: '/fee/gift-applys',
    ...
    events: {
      ...
      showEditBillPanel: function(index) {
        ...
        viewModel.pk = viewModel.complexList.getValue('ext01'); // ext01 是个扩展字段
      ...
      detail: function() {
        ...
        setTimeout(function() {
          ...
          var id = curRow.getValue("id");
          viewModel.findByParentid(id);
          viewModel.pk = curRow.getValue['ext01']; // ext01 是个扩展字段
          viewModel.queryAttach();
      ...
      submitCusReqForm: function(data, rowId) {
        ...
        var nodeJs = "/ocm-web/pages/fee/helpsaleapply/helpsaleapply.js";
        var billTypeCode = "GiftApply";
      ...
      lineopen: function() {
        ...
        if (allSelectRows.length < 1) {
          ...
          $._ajax({
            url: appCtx + '/gift-apply-details/open',
            type: 'post',
            data: {
              ids: ids.join(",")
            },
      ...
      lineclose: function() {
        ...
        if (allSelectRows.length < 1) {
          ...
          $._ajax({
            url: appCtx + '/gift-apply-details/close',
            type: 'post',
            data: {
              ids: ids.join(",")
            },
      ...
      viewModel.ItemRefList.on("productref.valuechange", function(obj) {
        ...
        if (refValues && refValues.length > 0) {
          for (var i = 0; i < refValues.length; i++) {
            ...
            var row = viewModel.complexItems.getRowByField("id", id);
            if (!row || row.status == u.Row.STATUS.FALSE_DELETE) {
              ...
              newrow.setValue("specification", refValues[i].specification);
              newrow.setValue("unitId", refValues[i].basicUnitId);
            }
          }
        }
      });
      viewModel.complexItems.on("mny.valuechange", function(obj) {
        if (obj.newValue == '') obj.newValue = '0'
        if (!obj.newValue) return;
        if (obj.newValue < 0) {
          obj.rowObj.setValue('mny', '0');
          return;
        }
        var allRows = viewModel.complexItems.getAllRows(),
          tMny = parseFloat(obj.newValue);
        for (var i = 0; i < allRows.length; i++) {
          if (obj.rowId != allRows[i].rowId && allRows[i].getValue('mny')) {
            tMny += parseFloat(allRows[i].getValue('mny'));
          }
        }
