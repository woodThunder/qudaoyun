define(['ocm_simpleview', './meta.js', 'ocm_common', "text!./supplierSchedule.html"], function (simpleview, model, common, tpl) {
  var viewModel;
  var view = simpleview.extend({
    ...
    baseData: {
      ...
      simpleList: new u.DataTable(model.options.metas.supplierSchedulemeta),
    ...
    events: u.extend({}, simpleview.prototype.events, {
      ...
      //批量调整供应商
      batchSupplier: function (data, rowId) {
        var rows = viewModel.simpleList.getSelectedRows();
        if (rows && rows.length > 0) {
          var title = "批量调整供应商"
          viewModel.dialogcardcomp2.show(title, "500px", viewModel.batchSupplierOk);
        } else {
