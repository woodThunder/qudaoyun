define(['ocm_simpleview', './meta.js', 'ocm_common'], function (simpleview, model,common) {
  ...
  var view = simpleview.extend({
    ...
    baseData: {
      ...
      simpleList: new u.DataTable(model.options.metas.shipperStrategymeta),
    ...
    rendertype: u.extend({}, common.rendertype, {
      ...
      dimensionRender: function (obj) {
        obj.element.innerHTML = (obj.value == 1 || obj.value == '1'? "四级收货地址" : "商品或产品线");
      },
      branchMethodRender: function (obj) {
        if(obj.value) {
          obj.element.innerHTML = (obj.value == 1 ? "单仓" : "分仓");
        }
      }
    }),
    ...
    afterCreate: function() {
      viewModel.dialogcardcomp.viewModel.params.on("logisticsModeCode.valuechange", function(obj) {
        var comp = viewModel.dialogcardcomp.app.getComp("branchMethodId");
        if (obj.newValue == '2' || obj.newValue == 2) {
          comp.setEnable(true);
          comp.validate.required = true;
          comp.validate.nullMsg = "不能为空！";
        } else {
          comp.setEnable(false);
          viewModel.dialogcardcomp.viewModel.params.setValue("branchMethod", "");
