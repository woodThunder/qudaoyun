define(['text!./clearinventory.html', 'ocm_common', 'searchbox', './meta.js', 'ocm_global'], function(tpl, common, searchbox) {
	'use strict'
	...
	rendertype = {
		...
		operation: function(obj) {
			var dataTableRowId = obj.row.value['$_#_@_id'];
			var confirmClearFun = "data-bind=click:confirmClear.bind($data," + obj.rowIndex + ")";
			var returnClearFun = "data-bind=click:returnClear.bind($data," + obj.rowIndex + ")";
	...
	events = {
		...
		confirmClear: function (data){
			...
			if(rows && rows.length > 0) {
				common.dialog.confirmDialog({
					...
					onOk: function() {
						$._ajax({
							url: appCtx + viewModel.baseurl + "/confirm-clean",
							type: "post",
							data: {
								ids: ids.join(",")
							},
		...
		returnClear: function (data){
			...
			if(rows && rows.length > 0) {
				...
				okButton.off().on('click', function() {
					...
					viewModel.confirmReturnClear("/rollback-clean", rows, returnReason);
				});
		...
		//确认退回
		confirmReturnClear: function (url, rows, reason){
			var ids = [];
			for(var i = 0; i < rows.length; i++) {
				ids.push(rows[i].getValue("id"));
			}
		...
		search: function(reindex) {
			...
			queryData["search_EQ_stockStatus"] = "3";
			$.ajax({
				type: "get",
