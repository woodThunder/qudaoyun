define(['text!./delivery.html', 'ocm_common', 'searchbox', 'editcard', 'billfooter', './meta.js', 'ocm_global'], function(tpl, common, searchbox, editcard, billfooter) {
	'use strict'
	var app, baseData, events, rendertype, viewModel, singledocSearch, singledoceidt, footer, popupDialog;
	//   var appCtx = '/occ-b2c';
	var BILLPANELSTATUS = {
		ADD: "add",
		EDIT: "edit",
	...
	baseData = {
		...
		remarkserviceData: new u.DataTable(remarkMeta),
	...
	events = {
		remarkEvt: function(rowIndex, rowId) {
			...
			if (selectedRows && selectedRows.length == 1) {
				...
				okButton.unbind("click").click(function() {
					self.addSaveRemark(rowData, id, vnote, remarkRow);
				});
		...
		sendsreturn: function() {
			...
			$._ajax({
				type: "post",
				url: appCtx + "/b2c/intercept-delivery/pushSaveOutReturn",
				// dataType: "json",
				// data: queryData,
				data: {
					deliveryIds: ids
		...
		interceptreturn: function() {
			...
			$._ajax({
				type: "post",
				url: appCtx + "/b2c/intercept-delivery/intercept-return",
				// dataType: "json",
				// data: queryData,
				data: {
					deliveryIds: ids
		...
		filllogistic: function() {
			...
			$._ajax({
				type: "post",
				url: appCtx + "/b2c/intercept-delivery/fill-logistic",
				// dataType: "json",
				// data: queryData,
				data: {
					deliveryIds: ids
		...
		search: function(reindex) {
			...
			$._ajax({
				...
				success: function(data) {
					...
					viewModel.OrderLogs.removeAllRows();
					viewModel.OrderProducts.removeAllRows();
					viewModel.OrderLogs.totalRow(0);
					viewModel.OrderLogs.totalPages(0);
				}
			});
			viewModel.OrderProducts.removeAllRows();
			viewModel.OrderLogs.removeAllRows();
		...
		fillData: function(id) {
			$._ajax({
				...
				data: {
					deliveryId: id,
					logPageSize: viewModel.OrderLogs.pageSize()
				},
				async: false,
				success: function(data) {
					var products = data.products;
	...
	function appInit(element, params) {
		...
		$._ajax({
			...
			async: false,
			data: {
				enumClassName: "com.yonyou.ocm.b2c.enums.DeliveryEnumCategory$DeliveryReturnStatusEnum"
			},
			success: function(data) {
		...
		singledocSearch = new searchbox(
			$("#PromoActivity-searchcontent")[0], [{
				...
				label: "退货状态",
				dataSource: viewModel.sendStatusSrc
			}, {
				type: "text",
				key: "buyerId",
				label: "顾客ID",
				...
				key: "isLock",
				label: "冻结标识",
				dataSource: viewModel.frozenFlags
			}]);
