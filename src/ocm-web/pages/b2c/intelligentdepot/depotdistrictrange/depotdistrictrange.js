define(['text!./depotdistrictrange.html', 'ocm_common', 'ocm_baseview', './meta.js'], function(tpl, common, baseview, model) {
  'use strict'
  ...
  var view = baseview.extend({
    ...
    afterCreate: function() {
      // 列表查询数据(无查询条件)
      viewModel.search();
      viewModel.depotDistrictList.on("storId.valuechange", function(obj) {
        // 清空参照时不增行
        if (!obj.newValue) {
          return;
        }
        var refer = $("[id^='refContainerstorId']").data("uui.refer");
        var refValues = refer.values;
        if (refValues) {
          viewModel.depotDistrictList.setValue('storCode', refValues[0].refcode);
          viewModel.depotDistrictList.setValue('storName', refValues[0].refname);
          viewModel.depotDistrictList.setValue('storId', refValues[0].refpk);
        }
      });
