define(['text!./supplierdistrictrange.html', 'ocm_common', 'ocm_baseview', './meta.js'], function(tpl, common, baseview, model) {
  'use strict'
  ...
  var view = baseview.extend({
    ...
    model: model,
    baseData: {
      baseurl: '/route/supplier-coverages',
    ...
    afterCreate: function() {
      ...
      // 列表查询数据(无查询条件)
      viewModel.search();
      viewModel.depotDistrictList.on("supplierId.valuechange", function(obj) {
        // 清空参照时不增行
        if (!obj.newValue) {
          return;
        }
        var refer = $("[id^='refContainersupplierId']").data("uui.refer");
        var refValues = refer.values;
        if (refValues) {
          viewModel.depotDistrictList.setValue('supplierCode', refValues[0].refcode);
          viewModel.depotDistrictList.setValue('supplierName', refValues[0].refname);
          viewModel.depotDistrictList.setValue('supplierId', refValues[0].refpk);
        }
      });
