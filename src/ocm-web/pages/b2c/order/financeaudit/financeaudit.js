define(['text!./financeaudit.html', 'ocm_common', 'ocm_baseview', './meta.js', 'ocm_ordertrack'], function(tpl, common, baseview, model, ordertrack) {
  'use strict'
  ...
  var view = baseview.extend({
    ...
    rendertype: {
      operation: function(obj) {
        ...
        if (billStatus == '04') {
          var editfun = "data-bind=click:financeAuditClick.bind($data," + obj.rowIndex + "," + dataTableRowId + ")";
          ...
          var editfun = "data-bind=click:cancelFinanceAuditClick.bind($data," + obj.rowIndex + "," + dataTableRowId + ")";
          obj.element.innerHTML = '<div class="ui-handle-icon">' +
            '<span class="ui-handle-word">' +
            '<a href="#" class="" ' +
            editfun +
    ...
    events: {
      //财审
      financeAuditClick: function(data, rowId) {
        ...
        if (rows && rows.length > 0) {
          ...
          $._ajax({
            type: "post",
            url: appCtx + viewModel.baseurl + '/financial-audit',
            data: {
              orderIds: ids
            },
      ...
      //取消财审
      cancelFinanceAuditClick: function(data, rowId) {
        if (typeof(data) == 'number') {
          viewModel.processOrderList.setRowSelectbyRowId(rowId);
        }
        var ids = [];
        ...
        if (rows && rows.length > 0) {
          ...
          $._ajax({
            type: "post",
            url: appCtx + viewModel.baseurl + '/anti-financial-audit',
            data: {
              orderIds: ids
            },
            success: function(data) {
      ...
      search: function(reindex) {
        ...
        if (!queryData["search_IN_billStatus"]) {
      ...
      $._ajax({
        ...
        success: function(data) {
          ...
          for (var i = 0; i < newarray.length; i++) {
            if (newarray[i].value == "04" || newarray[i].value == "05") {
