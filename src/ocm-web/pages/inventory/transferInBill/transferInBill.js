define(['text!./transferInBill.html', 'ocm_common', 'ocm_baseview', './meta.js'], function(tpl, common, baseview, model) {
    'use strict';
    ...
    var viewModel,
        ...
        view = baseview.extend({
            ...
            model: model,
            baseData: {
                baseurl: '/stock/transfer-in-bills',
                ...
                defaultItemPage: new u.DataTable(model.options.metas.defaultItemPageDatas), //子页table
                ...
                //新增 参照 搜索
                referTransferOutSearchParam: new DataTable(model.options.metas.referTransferOutSearch),

                //拉单 调拨出
                referTransferOutList: new u.DataTable(model.options.metas.referTransferOut),
                //拉单 对应商品
                referTransferOutItemList: new u.DataTable(model.options.metas.referTransferOutItem),
                //拉单 已选择
                selectedreferList: new u.DataTable(model.options.metas.referTransferOut),
                selectedreferListItem: new u.DataTable(model.options.metas.referTransferOutItem),
            ...
            events: { //事件
                ...
                showAddRefer: function () {
                    ...
                    viewModel.referTransferOutSearchParam.removeAllRows();
                    viewModel.removeAllreferRows();
                    if (!viewModel.refertransferoutdialog) {
                        viewModel.refertransferoutdialog = u.dialog({
                            id: 'dialog_refertransferout',
                            content: "#dialog_refertransferout",
                            hasCloseMenu: true,
                            width: "85%"
                        });
                        var closefunc = function () {
                            viewModel.referTransferOutSearchParam.removeAllRows();
                            viewModel.referTransferOutList.removeAllRows();
                            viewModel.referTransferOutItemList.removeAllRows();
                            viewModel.refertransferoutdialog.close();
                        }
                        ...
                        viewModel.refertransferoutdialog.show();
                    }
                },
                ...
                confirmReferTransferBill: function () {
                    ...
                    for (var i = 0; i < parentArr.length; i++) {
                        ...
                        parentArr[i].transferBillOutItems = [];
                        var BomsArr = parentArr[i].transferOutBillItemBoms;
                        parentArr[i].transferOutBillItemBoms = [];
                        for (var j = 0; j < itemArr.length; j++) {
                            itemArr[j].persistStatus = "nrm";
                            if (itemArr[j].transferOutBillId == parentArr[i].id) {
                                parentArr[i].transferBillOutItems.push(itemArr[j]);
                                for (var k = 0; k < BomsArr.length; k ++) {
                                    ...
                                    if (BomsArr[k].parentGoodsId == itemArr[j].goodsId && BomsArr[k].parentRowNum == itemArr[j].rowNum) {
                                        parentArr[i].transferOutBillItemBoms.push(BomsArr[k]);
                                    }
                                }
                            }
                        }
                ...
                queryChildArr: function (parentArr) {
                    $._ajax({
                        type: "post",
                        url: appCtx + viewModel.baseurl + "/translate-allocation-in",
                        async: false,
                        ...
                        success: function (data) {
                            ...
                            viewModel.defaultItemPage.setSimpleData(data[0].transferInBillItems, {
                                unSelect: true
                            });
                            viewModel.BomItemPage.setSimpleData(data[0].transferInBillItemBoms, {
                                unSelect: true
                            });
                            viewModel.calcItemTotal();
                            viewModel.calcItemShouldInTotal();
                        }
                    })
                },
                ...
                pageChangeRefer: function (index) {
                    viewModel.referTransferOutList.pageIndex(index);
                    viewModel.searchRefer();
                },

                //页码改变时的回调函数
                sizeChangeRefer: function (size) {
                    viewModel.referTransferOutList.pageSize(size);
                    viewModel.searchRefer(true);
                },

                //页码改变时的回调函数
                pageChangeRefer3: function (index) {
                    viewModel.referTransferOutItemList.pageIndex(index);
                    viewModel.searchRefer();
                },

                //页码改变时的回调函数
                sizeChangeRefer3: function (size) {
                    viewModel.referTransferOutItemList.pageSize(size);
                    viewModel.searchRefer(true);
                },
                ...
                searchRefer: function (reindex) {
                    if (reindex) {
                        viewModel.referTransferOutList.pageIndex(0);
                    }
                    viewModel.referTransferOutList.removeAllRows();
                    var queryData = viewModel.searchcomp2.getDataWithOpr ? viewModel.searchcomp2.getDataWithOpr() : {};
                    var pageSize = viewModel.referTransferOutList.pageSize();
                    var pageNumber = viewModel.referTransferOutList.pageIndex();
                    queryData.page = pageNumber;
                    queryData.size = pageSize;
                    queryData["stockOrgIn"] = queryData.search_EQ_receiveStorageOrgId;
                    queryData["inStorage"] = queryData.search_EQ_receiveStorageId;
                    ...
                    $._ajax({
                        type: "get",
                        url: window.pathMap.inventory + '/stock/transfer-out-bills/get-by-param',
                        dataType: "json",
                        data: queryData,
                        success: function (data) {
                            viewModel.referTransferOutList.setSimpleData(data.content, { unSelect: true });
                            viewModel.referTransferOutList.totalRow(data.totalElements);
                            viewModel.referTransferOutList.totalPages(data.totalPages);
                ...
                updateSelectedIndices: function () {
                    ...
                    if (selectedRows && selectedRows.length > 0) {
                        ...
                        rows = viewModel.referTransferOutList.getAllRows();
                        ...
                        if (selectedIndices.length > 0) {
                            viewModel.referTransferOutList.setRowsSelect(selectedIndices);
                        }
                    } else {
                        return
                    }
                ...
                referSelectHandle: function (obj) {
                    // 获取商品行信息
                    var items = obj.rowObj.value.transferBillOutItems.getSimpleData();
                    var selectData = viewModel.referTransferOutList.getCurrentRow().getSimpleData();
                    viewModel.selectedreferList.setSimpleData(selectData, {
                        unSelect: true
                    });
                    viewModel.referTransferOutItemList.setSimpleData(items, {
                        unSelect: true
                    });
                },
                // 拉单== 调拨单取消选中
                referUnSelectHandle: function (obj) {
                    ...
                    var itemrows = viewModel.referTransferOutItemList.getAllRows();
                    ...
                    for (var i = itemrows.length - 1; i >= 0; i--) {
                        if (itemrows[i].getValue("transferBillId") == id) {
                            viewModel.referTransferOutItemList.removeRows([i], {forceDel: true});
                        }
                    }
                ...
                referSelectItemHandle: function (obj) {
                    var id = obj.rowObj.value.id;
                    var selectedRows = viewModel.referTransferOutItemList.getSelectedRows();
                ...
                showEditBillPanel: function(index, rowId) {
                    ...
                    viewModel.findByParentid(id, function(data) {
                        data.transferInBillItems.forEach(function(item) {
                            item.batchCodeName = item.batchCodeCode;
                            item.batchNumName = item.batchNumCode;
                        });
                        data.transferInBillItemBoms.forEach(function(item) {
                            item.batchCodeName = item.batchCodeCode;
                            item.batchNumName = item.batchNumCode;
                        });
                        viewModel.defaultPageCard.setSimpleData(data);
                        var dpDatas = data.transferInBillItems;

                        viewModel.defaultItemPage.setSimpleData(data.transferInBillItems, {
                            unSelect: true
                        });
                        viewModel.BomItemPage.setSimpleData(data.transferInBillItemBoms, {
                            unSelect: true
                        });
                        // 获得当前最大行号
                ...
                seteidtCondition: function(data) {
                    var pkOrgId = viewModel.defaultPageCard.getValue("stockOrgInId");
                ...
                saveBill: function() {
                    ...
                    if (resultData) {
                        ...
                        // debugger;
                        defaultPageData.transferInBillItems = defaultItemPageData;
                        defaultPageData.transferInBillItemBoms = BomItemsData;
                ...
                calcItemShouldInTotal: function() {
                    var rows = viewModel.defaultItemPage.getAllRealRows();
                    var totalfactInAmount = 0;
                    if (u.isArray(rows) && rows.length > 0) {
                        for (var i = 0; i < rows.length; i++) {
                            totalfactInAmount += parseFloat(rows[i].getValue("shouldInNum") || 0);
                        }
                        totalfactInAmount = viewModel.formater.format(totalfactInAmount);
                    }
                    viewModel.defaultPageCard.setValue("totalShouldInNum", totalfactInAmount);
                },
                calcItemTotal: function() {
                    ...
                    viewModel.defaultPageCard.setValue("totalFactInNum", totalfactInAmount);
                },
                ...
                setValueToBom: function(obj) {
                    ...
                    for (var i = 0; i < allrows.length; i++) {
                        ...
                        if (allrows[i].getValue("parentRowNum") == parentRowNum) {
                            allrows[i].setValue(idField, obj.newValue);
                            if (obj.newValue) {
            ...
            afterCreate: function() {
                ...
                // 转库入库组织和仓库联动
                viewModel.defaultPageCard.on("stockOrgInId.valuechange", function(obj) {
                    ...
                    if (pkOrgId) {
                        ...
                        // 仓库不可点，选择完库存组织后可点击
                        $("#inCardStorageId").attr('placeholder', "");
                        ...
                        // 仓库不可点，选择完库存组织后可点击
                        $("#inCardStorageId").attr('placeholder', "请先选择库存组织");
                ...
                viewModel.defaultPageCard.on("storageId.valuechange", function(obj) {
                    if (obj.newValue) {
                        ...
                        if (refer.values && refer.values.length > 0) {
                            ...
                            if (!viewModel.defaultPageCard.getValue("stockOrgInId") ||
                                viewModel.defaultPageCard.getValue("stockOrgInId") != trueValue.inventoryOrgId) {
                                viewModel.defaultPageCard.getFocusRow().setValue("stockOrgInId", trueValue.inventoryOrgId, true);
                            }
                            // 更新货位管理状态
                ...
                // 数量联动金额
                viewModel.defaultItemPage.on("factInNum.valuechange", function(obj) {
                    var row = obj.rowObj;
                ...
                viewModel.defaultItemPage.on("unitPrice.valuechange", function(obj) {
                    var row = obj.rowObj;
                    if (obj.newValue) {
                        var factInNum = row.getValue("factInNum") || 0;
                        var amount = viewModel.formater.format(parseFloat(obj.newValue) * parseFloat(factInNum));
                        row.setValue("amountMoney", amount);
                    }
                ...
                viewModel.BomItemPage.on("unitPrice.valuechange", function(obj) {
                    ...
                    var amount = viewModel.formater.format(parseFloat(unitPrice) * parseFloat(row.getValue("factInNum")));
                    row.setValue("amountMoney", amount);
                ...
                viewModel.defaultItemPage.on("valuechange", function(obj) {
                    ...
                    if (obj.field === "batchNumId" ||
                        ...
                        if (obj.field === "goodsPositionId") {
                            if (obj.rowObj.data['goodsPositionId'].meta) {
                                obj.rowObj.setValue("goodsPositionName", obj.rowObj.data['goodsPositionId'].meta.display);
                            }
                        }
                    }
                    console.log(obj.rowObj.getValue("goodsPositionName"))
                });
                // viewModel.defaultItemPage.on("goodsPositionId.valuechange", function(obj) {
                //     if (!obj.newValue) return;
                //     var allrows = viewModel.BomItemPage.getAllRealRows();
                //     var parentRowNum = obj.rowObj.getValue("rowNum");
                //     console.log(obj.rowObj.data['goodsPositionId'].value)
                //     for (var i = 0; i < allrows.length; i++) {
                //         var nameField = obj.field.replace("Id", "Name");
                //         var idField = obj.field;
                //         var display = obj.rowObj.data[idField].meta
                ...
                //搜索条件 库存组织仓库过滤
                viewModel.searchcomp.viewModel.params.on("stockOrgIn.valuechange", function(obj) {
                    if (obj.newValue != undefined && obj.newValue != "") {
                        var stockOrgId = {
                            "EQ_inventoryOrg.id": obj.newValue
                        };
                        $("#warehouseIn").attr("data-refparam", JSON.stringify(stockOrgId));
                    } else {
                        $("#warehouseIn").attr("data-refparam", '{"EQ_inventoryOrg.id":""}');
