define(['text!./allocation.html', 'ocm_common', 'ocm_baseview', './meta.js', '../../flow/bpmapproveref/bpmopenbill.js'], function (tpl, common, baseview, model, bpmopenbill) {
    'use strict';
    var viewModel, app;
    var BILLPANELSTATUS = {
        ADD: "add",
    ...
    var view = baseview.extend({
        ...
        model: model,
        baseData: {
            baseurl: '/stock/allocation-bill',
            applicationUrl: '/stock/allocation-bill',
            ...
            //拉单 门店要货订单
            referB2cReqorderList: new u.DataTable(model.options.metas.referB2cReqOrder),
            //拉单 对应商品
            referB2cReqorderitemList: new u.DataTable(model.options.metas.referB2cReqorderitem),
            //拉单 已选择
            selectedreferList: new u.DataTable(model.options.metas.referB2cReqOrder),
            selectedreferListItem: new u.DataTable(model.options.metas.referB2cReqorderitem),
            //新增 参照 搜索
            referB2cReqorderSearchParam: new DataTable(model.options.metas.referB2cReqorderSearch),
        ...
        rendertype: {
            ...
            beforeEditCheck: function (obj) {
                ...
                // 出入库仓库
                if (obj.colIndex == gridObj.getIndexOfColumn(gridObj.getColumnByField("outStorageId")) || obj.colIndex == gridObj.getIndexOfColumn(gridObj.getColumnByField("inStorageId"))) {
                    var pkOrgId = viewModel.transferCard.getValue("pkOrgId");
                    var pkOrgInId = viewModel.transferCard.getValue("pkOrgInId");
                    viewModel.transferItems.setMeta("outStorageId", "refparam", '{"EQ_isEnable":"1", "EQ_inventoryOrg.id":"'+ pkOrgId +'"}');
                    viewModel.transferItems.setMeta("inStorageId", "refparam", '{"EQ_isEnable":"1", "EQ_inventoryOrg.id":"'+ pkOrgInId +'"}');
                }
                return true
            },
            beforeEditGoodsposiCheck: function (obj) {
                var gridObj = obj.gridObj;
                // 判断当前货位是否可编辑
                if (obj.colIndex == gridObj.getIndexOfColumn(gridObj.getColumnByField("outPositionId"))) {
        ...
        events: {
            ...
            search: function (reindex) {
                ...
                var billType = "Allocation";
                queryData.page = pageNumber;
                queryData.size = pageSize;
                queryData.search_EQ_billType = billType;
                $._ajax({
                    type: "get",
            ...
            setDefaultValue: function (row) {
                ...
                row.setValue("state", 0);
                row.setValue("billType", "Allocation");
                row.setValue("billTranTypeId", "Allocation");
            },
            //进入修改单据页
            ...
            saveBill: function () {
                ...
                if (result) {
                    if (!viewModel.checkEmpty(viewModel.transferCard, viewModel.transferItems, true, true)) {
                        return
                    }
                    if(viewModel.transferCard.getValue("pkOrgId") == viewModel.transferCard.getValue("pkOrgInId")) {
                        toastr.error("出库库存组织与入库库存组织不能相同");
                        return false;
                    }
                    // 组装数据
            ...
            checkEmpty: function (mainDt, subDt, ifcheckgoodsposi, ifcheckbatchnum) {
                ...
                if (allItemRows.length > 0) {
                    allItemRows.forEach(function (item) {
                        var ifSlotManage = item.getValue("ifSlotManage");
                        ...
                        if (!item.getValue("outPositionId") && parseFloat(ifSlotManage) === 1) {
            ...
            approve: function (obj) {
                //approve : 审批  cancelApprove: 取消审批   oneKeyTrans：一键调拨
                var requsetUrl,
                    bsCode,
                    bsName,
                    data;
            ...
            //确定 新增
            confirmReferb2creqorder: function () {
                $("#tab-edit-bom").hide();
                $("#tab-edit-goods").show();
                var parentArr = viewModel.referB2cReqorderList.getSimpleData({
                    type: 'select'
                });
                var itemArr = viewModel.referB2cReqorderitemList.getSimpleData({
                    type: 'select'
                });
                if (!itemArr.length) {
                ...
                for (var i = 0; i < parentArr.length; i++) {
                    parentArr[i].reqOrderItemList = [];
                    for (var j = 0; j < itemArr.length; j++) {
                        if (itemArr[j].reqOrderId == parentArr[i].id) {
                            parentArr[i].reqOrderItemList.push(itemArr[j]);
                        }
                    }
                }
                parentArr.forEach(function (item) {
                    selBomsArr.push(item.reqOrderItemBomList)
                });
                viewModel.currowNum(0);
                viewModel.currowBomNum(0);
                ...
                viewModel.goBillPanel();
                viewModel.billPanelStatus(CONST.BILLPANELSTATUS.EDIT);
                viewModel.referpurchaseorderdialog.close();//todo
                viewModel.removeAllreferRows();
            },
            //根据条件搜索数据(根据是否点击搜索按钮，判断是否需要重置页码)  新增参照
            searchRefer: function (reindex) {
                if (reindex) {
                    viewModel.referB2cReqorderList.pageIndex(0);
                }
                viewModel.referB2cReqorderList.removeAllRows();
                var queryData = viewModel.searchcomp2.getDataWithOpr ? viewModel.searchcomp2.getDataWithOpr() : {};
                var pageSize = viewModel.referB2cReqorderList.pageSize();
                var pageNumber = viewModel.referB2cReqorderList.pageIndex();
                queryData.page = pageNumber;
                queryData.size = pageSize;
                ...
                // queryData["reqordercode"] = queryData.search_LIKE_purchaseCode;
                ...
                $._ajax({
                    type: "get",
                    url: window.pathMap.b2c + '/b2c/req-orders',
                    dataType: "json",
                    data: queryData,
                    success: function (data) {
                        viewModel.referB2cReqorderList.setSimpleData(data.content, {
                            unSelect: true
                        });
                        viewModel.referB2cReqorderList.totalRow(data.totalElements);
                        viewModel.referB2cReqorderList.totalPages(data.totalPages);
            ...
            //新增 参照
            showAddReferB2CReq: function () {
                viewModel.searchcomp2.clearSearch();
                viewModel.referB2cReqorderSearchParam.removeAllRows();
                viewModel.removeAllreferRows();
                if (!viewModel.referb2creqorderdialog) {
                    viewModel.referb2creqorderdialog = u.dialog({
                        id: 'dialog_referb2creqorder',
                        content: "#dialog_referb2creqorder",
                        hasCloseMenu: true,
                        width: "85%"
                    });
                    var closefunc = function () {
                        viewModel.referB2cReqorderSearchParam.removeAllRows();
                        viewModel.referB2cReqorderList.removeAllRows();
                        viewModel.referB2cReqorderitemList.removeAllRows();
                        viewModel.referb2creqorderdialog.close();
                    }
                    var cancelButton = $("#dialog_referb2creqorder .J-cancel");
                    var closeButton = $("#dialog_referb2creqorder .u-msg-close");
                    cancelButton.off().on('click', closefunc);
                    closeButton.on('click', closefunc);
                } else {
                    viewModel.referb2creqorderdialog.show();
                }
            },
            ...
            referSelectHandle: function (obj) {
                ...
                viewModel.selectedreferListItem.removeAllRows();
                var listArr = [];
                var selectedRows = viewModel.referB2cReqorderitemList.getSelectedRows();
                if (selectedRows && selectedRows.length > 0) {
                    for (var i = 0; i < selectedRows.length; i++) {
                        var listData = selectedRows[i].getSimpleData();
                        listArr.push(listData);
                        viewModel.referB2cReqorderitemList.addSimpleData(listData.reqOrderItemList);
                        viewModel.referB2cReqorderitemList.setAllRowsSelect();
                        var subIds = listData.reqOrderItemList.map(function (item) {
                            return item.id;
                        })
            ...
            referUnSelectHandle: function (obj) {
                ...
                // var itemId = obj.rowObj.value.reqOrderItemList.getValue("id");
                var rows = viewModel.selectedreferList.getAllRows();
                var itemrows = viewModel.referB2cReqorderitemList.getAllRows();
                ...
                for (var i = itemrows.length - 1; i >= 0; i--) {
                    if (itemrows[i].getValue("reqOrderId") == id) {
                        viewModel.referB2cReqorderitemList.removeRows([i], {forceDel: true});
                    }
                }
            },
            ...
            showTransferOut: function () {
                ...
                if (status != '4') {
                    if (status < '4') {
                        ...
                        toastr.warning('已执行过调拨出库操作,不能再次转出');
                    }
                    return false;
                }
                    ...
                    if (data.transferBillItems && data.transferBillItems.length > 0) {
                        newSubData = data.transferBillItems.map(function (item) {
                            return u.extend({}, {
                                ...
                                firstBillType: "Allocation",
                                srcBillCode: data.id,
                                srcBillBcode: item.id,
                                srcBillType: "Allocation"
                            });
                        });
                    }
            ...
            setOutCondition: function () {
                ...
                viewModel.transferOutItems.setMeta("outPositionId", "refparam",
            ...
            showTransferIns: function (obj) {
                ...
                viewModel.findByParentid(id, function (data) {
                    ...
                    var newData = u.extend({}, {
                        ...
                        remark: data.remark,
                        billType: "AllocationIn",
                        billTranTypeId: "AllocationIn"
                    });
                    var newSubData = [];
                    if (data.transferBillItems && data.transferBillItems.length > 0) {
                        newSubData = data.transferBillItems.map(function (item) {
                            return u.extend({}, {
                                ...
                                factInNum: 0, //实入数量(来源于调拨单表体的累计已出库数量)
                                unitPrice: item.unitPrice, //单价
                                amountMoney: item.amountMoney, //金额
                                batchNumId: item.batchNumId, //批次号
                                batchNumCode: item.batchNumCode, //批次号
                    ...
                    if (data.transferBillItems && data.transferBillItems.length > 0) {
                        newSubData = data.transferBillItems.map(function (item) {
                            return u.extend({}, {
                                ...
                                unitName: item.unitName, //单位
                                shouldInNum: item.transferNum, //应入数量（来源应转数量）
                                factInNum: item.totalOutNum, //实入数量(来源于调拨单表体的累计已出库数量)
                                price: item.price, //单价
                                amountMoney: item.amountMoney, //金额
                                batchNumId: item.batchNumId, //批次号
                                batchNumCode: item.batchNumCode, //批次号
            ...
            setInCondition: function () {
                ...
                viewModel.transferInItems.setMeta("outPositionId", "refparam",
                    '{"EQ_wareHouse":"' + storehouseId + '","EQ_isFinal":"1"}');
            },
            //参照选择批量新增子表（销售产品）
        ...
        afterCreate: function () {
                ...
                if (obj.oldValue && obj.oldValue != obj.newValue) {
                    ...
                    if (itemrows && itemrows.length > 0) {
                        itemrows.forEach(function (row) {
                            if (row.getValue("outPositionId")) {
                                row.setValue("outPositionId", null);
                                row.setValue("goodsPositionName", null);
                                row.setMeta("outPositionId", 'display', null);
                            }
                        })
                    }
                }
            ...
            viewModel.transferItems.on("valuechange", function (obj) {
                if (obj.field === "batchNumId" ||
                    ...
                    obj.field === "outStorageId" ||
                    obj.field === "inStorageId" ||
                    obj.field === "outPositionId" ||
