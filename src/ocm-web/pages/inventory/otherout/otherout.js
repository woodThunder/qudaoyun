define(['text!./otherout.html', 'ocm_common', 'ocm_baseview', './meta.js'], function (tpl, common, baseview, model) {
	'use strict'
	var viewModel, app;
	var BILLPANELSTATUS = {
		ADD: "add",
	...
	var view = baseview.extend({
		...
		rendertype: {
			...
			beforeEditCheck: function (obj) {
				...
				if (obj.colIndex == gridObj.getIndexOfColumn(gridObj.getColumnByField("goodsPositionId"))) {
					// 先判断有没有仓库
					if (viewModel.complexList.getValue("stockOutStorageId")) {
						// 判断是否启用货位管理
		...
		events: {
			...
			search: function (reindex) {
				...
				if (goods) {
					queryData["search_EQ_otherOutBillItems.goods"] = goods;
					delete queryData.search_EQ_goods;
				}
			...
			showAddBillPanel: function () {
				...
				curRow.setValue("tranTypeId", "OtherOut");
				curRow.setValue("tranTypeName", "其它出库");
				...
				viewModel.complexList.setMeta("stockOutStorageId", "enable", false);
				$("#stockOutStorageref").attr('placeholder', "请先选择库存组织");
			...
			seteidtCondition: function () {
				...
				var stockOutStorageId = viewModel.complexList.getValue("stockOutStorageId");
				viewModel.changeCondition("stockOutStorageref", {
					"EQ_isEnable": "1"
				}, {
					"EQ_inventoryOrg.id": stockOrgId
				})
				viewModel.complexItems.setMeta("goodsPositionId", "refparam", '{"EQ_wareHouse":"' + stockOutStorageId + '","EQ_isFinal":"1"}');
			},
			//进入复制单据页
			...
			findByParentid: function (id) {
				$._ajax({
					...
					success: function (data) {
						data.otherOutBillItems.forEach(function (item) {
							item.batchCodeName = item.batchCodeCode;
						});
						data.otherOutBillItemBoms.forEach(function (item) {
							item.batchCodeName = item.batchCodeCode;
						});
						viewModel.complexItems.setSimpleData(data.otherOutBillItems, {
							unSelect: true
						});
						viewModel.BomItems.setSimpleData(data.otherOutBillItemBoms, {
							unSelect: true
						});
					}
				})
			...
			saveBill: function () {
				...
				complexData.otherOutBillItems = complexItemsData;
				complexData.otherOutBillItemBoms = BomItemsData;
			...
			showAddItemsRef: function () {
				...
				if (curRow.stockOrgId && curRow.stockOutStorageId) {
		...
		afterCreate: function () {
					...
					for (var i = 0; i < refValues.length; i++) {
						...
						if (bomdata.length > 0) {
							...
							bomItem.forEach(function (item) {
								...
								item.factOutNum = item.childGoodsQty;
			...
			//实出数量
			viewModel.complexItems.on("factOutNum.valuechange", function (obj) {
				if (obj.oldValue != undefined && obj.oldValue != obj.newValue) {
					if (obj.newValue < 0.00) {
						toastr.warning("实出数量不能小于 0 ");
						obj.rowObj.setValue("factOutNum", 0.00);
					}
					obj.rowObj.setValue("amountMoney", viewModel.sumPrice(obj.rowObj));
				}
				var factOutNum = viewModel.complexItems.getValue("factOutNum");
				var shouldOutNum = viewModel.complexItems.getValue("shouldOutNum");
				//  if (!shouldOutNum || !factOutNum || factOutNum == 0 || shouldOutNum == 0) {
				if ((!shouldOutNum || shouldOutNum == 0) && (factOutNum || factOutNum != 0)) {
					obj.rowObj.setValue("shouldOutNum", factOutNum == "" ? 0 : factOutNum);
				}
				...
				viewModel.complexList.getCurrentRow().setValue("totalFactOutNum", getSum(amount));
			...
			//实出数量
			viewModel.complexItems.on("shouldOutNum.valuechange", function (obj) {
				if (obj.newValue < 0.00) {
					toastr.warning("实出数量不能小于 0 ");
					obj.rowObj.setValue("shouldOutNum", 0.00);
				}
				//联动bom数量
				var parentRowNum = obj.rowObj.getValue("rowNum");
				//获取全部bom信息
				...
				for (var i = 0; i < allrows.length; i++) {
					if (allrows[i].getValue("parentRowNum") == parentRowNum) {
                        var childQty = allrows[i].getValue("childGoodsQty") ? allrows[i].getValue("childGoodsQty") : 1;
                        var bomAmount = childQty * obj.newValue;
                        allrows[i].setValue("shouldOutNum", bomAmount)
                    }
				}
			});
			//单价
			...
			viewModel.complexList.on("stockOrgId.valuechange", function (obj) {
				...
				$("#stockOutStorageref").parent().attr("data-refparam", '{"EQ_isEnable":"1","EQ_inventoryOrg.id":"' + stockOrgId + '"}');
				if (stockOrgId) {
					viewModel.complexList.setMeta("stockOutStorageId", "enable", true);
					$("#stockOutStorageref").removeAttr('placeholder');
				} else {
					viewModel.complexList.setMeta("stockOutStorageId", "enable", false);
					$("#stockOutStorageref").attr('placeholder', "请先选择库存组织");
				}
			...
			// 仓库
			viewModel.complexList.on("stockOutStorageId.valuechange", function (obj) {
				var row = viewModel.complexList.getCurrentRow();
				var stockOrgId = row.getValue("stockOrgId");
				// if (stockOrgId) {
				...
				// };
				// 仓库变化时改变是否启用货位管理字段
				var Storageref = $("#refContainerstockOutStorageref").data("uui.refer");
				if (obj.newValue) {
