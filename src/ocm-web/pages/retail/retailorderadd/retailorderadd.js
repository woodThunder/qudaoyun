define(['text!./retailorderadd.html', 'ocm_common', 'ocm_baseview', './meta.js', 'ocm_ordertrack', "../../flow/bpmapproveref/bpmopenbill.js"], function(tpl, common, baseview, model, ordertrack, bpmopenbill) {
  'use strict'
  ...
  var view = baseview.extend({
    ...
    baseData: {
      ...
      //订单关闭
      isCloseCom: ko.pureComputed(function() {
        var enableStatus = viewModel.orderDetail.ref("close")();
        switch (enableStatus) {
          case '0':
            return "否";
          case '1':
    ...
    events: {
      ...
      //   var nodeJs = "/ocm-web/pages/fee/retailorderadd/retailorderadd.js";
      ...
      saveBill: function() {
        ...
        orderData.buyerPhone = parseInt(orderData.buyerPhone);
    ...
    afterCreate: function() {
      app = viewModel.app;
      viewModel.orderAdd.on("retailOrderCode.valuechange", function(obj) {
        if (!obj.newValue) return;
        obj.rowObj.setValue('pcode', obj.newValue);
      });
      var productRow = viewModel.ProductRef.createEmptyRow();
      viewModel.ProductRef.setRowFocus(productRow);
      var combineProductRow = viewModel.CombineProductRef.createEmptyRow();
      ...
      //门店联动 
      viewModel.orderAdd.on("shopId.valuechange", function(obj) {
        if (!obj.newValue) return;
        var refValues = $('div[id^="refContainershopId"]').data("uui.refer").values;
        if (refValues && refValues.length > 0) {
          obj.rowObj.setValue('pkOrgId', refValues[0].organizationId)
        }
      });
      //物流方式枚举
      $._ajax({
      ...
      $._ajax({
        ...
        async: false,
        data: {
          enumClassName: "com.yonyou.ocm.b2c.enums.SalesModelEnum"
        },
        success: function(data) {
          var newarray = common.dataconvert.toMap(data, "name", "code");
          viewModel.salesModelSrc = newarray;
        }
      });
      //平台状态枚举
      $._ajax({
