define(
  ["text!./retaildeliveryorder.html", "ocm_common", "ocm_baseview", "./meta.js", "../../flow/bpmapproveref/bpmopenbill.js"],
  function(tpl, common, baseview, model, bpmopenbill) {
    "use strict";
    var viewModel, app, appCtx = "/occ-b2c";
    var BILLPANELSTATUS = {
      ADD: "add",
      EDIT: "edit",
      COPY: "copy",
    ...
    var view = baseview.extend({
      ...
      baseData: {
        ...
        applicationUrl: "/b2c/order-arrange/",
        itemBomurl: "/b2c/order-item-boms",
        // 发货单
        ...
        deliveryOrderList: new u.DataTable(model.options.metas.deliveryOrder),
        deliveryOrderCard: new u.DataTable(model.options.metas.deliveryOrder),
        deliveryOrderDetailCard: new u.DataTable(model.options.metas.deliveryOrder),
        deliveryOrderItemsList: new u.DataTable(model.options.metas.deliveryItemsMeta),

        // deliveryOrderItemBomsList: new u.DataTable(model.options.metas.deliveryOrder_ItemBom),
        ...
        orderStatus: ko.pureComputed(function() {
          var orderStatusVal = viewModel.deliveryOrderList.ref("billStatus")();
      ...
      events: {
        ...
        resetdelivery: function() {
          ...
          // deliveryData.deliveryOrderItemBoms = deliveryItemBomsData;
          var url = "matching-warehouse";
          $._ajax({
            url: appCtx + viewModel.applicationUrl + url,
            type: "post",
            data: JSON.stringify(viewModel.confirmButDatas),
            contentType: "application/json; charset=utf-8",
            success: function(data) {
              // 匹配之后只需要更新子表定的数据 
              viewModel.deliveryOrderItemsList.setSimpleData(
        ...
        submitBillBpm: function(rows) {
          ...
          var nodeJs = "/ocm-web/pages/b2c/deliveryorder/deliveryorder.js";
        ...
        searchRefer: function(reindex) {
          ...
          // if (!queryData["search_EQ_shop"]) {
          //   toastr.warning("门店不能为空");
          //   return false;
          // }
        ...
        referToEdit: function(itemArr) {
          ...
          for (var i = 0; i < itemArr.deliveryProduct.length; i++) {
            ...
            var anNum = itemArr.deliveryProduct[i].deliveryNumber;
            totalNum += parseFloat(anNum || 0);
          }
          curRow.setValue("totalDeliveryNumber", totalNum);
        ...
        confirmReferSaleOrder: function() {
          ...
          viewModel.goBillPanel(
            function() {
              viewModel.confirmButDatas = itemArr;
      ...
      afterCreate: function() {
        ...
        viewModel.deliveryOrderCard.on("deliveryReceiverProvince.valuechange", function(obj) {
          var row = viewModel.deliveryOrderCard.getCurrentRow();
          var provinceId = row.getValue("deliveryReceiverProvince");
        ...
        //省市联动 
        viewModel.deliveryOrderCard.on("deliveryReceiverCity.valuechange", function(obj) {
          var row = viewModel.deliveryOrderCard.getCurrentRow();
          var cityId = row.getValue("deliveryReceiverCity");
          $("#district").attr("data-refparam", '{"EQ_isEnable":"1","EQ_areaLevel":"3","EQ_parent.id":"' + cityId + '"}');
          $("#district input").val('');
          $("#town input").val('');
        });
        //市区联动
        viewModel.deliveryOrderCard.on("deliveryReceiverDistrict.valuechange", function(obj) {
          var row = viewModel.deliveryOrderCard.getCurrentRow();
          var districtId = row.getValue("deliveryReceiverDistrict");
          $("#town").attr("data-refparam", '{"EQ_isEnable":"1","EQ_areaLevel":"4","EQ_parent.id":"' + districtId + '"}');
          $("#town input").val('');
        });
        //区镇联动
        viewModel.deliveryOrderCard.on("deliveryReceiverTown.valuechange", function(obj) {

        });
        viewModel.deliveryOrderItemsList.on('deliveryNumber.valuechange', function(obj) {
          if (!obj.newValue) return;
          var curRow = viewModel.deliveryOrderCard.getCurrentRow(),
            allRows = viewModel.deliveryOrderItemsList.getAllRows();
          var totalNum = 0;
          for (var i = 0; i < allRows.length; i++) {
            var anNum = allRows[i].getValue("deliveryNumber");
            totalNum += parseFloat(anNum || 0);
          }
          curRow.setValue('totalDeliveryNumber', totalNum);
        });
        ...
        viewModel.searchcomp2.viewModel.params.on("order--saleOrg.valuechange", function(obj) {
          ...
          if (!obj.newValue) {
            $("#search_customer").attr("placeholder", "请先选择销售组织").attr({
              "readonly": "readonly",
              "disabled": "disabled"
            });
          } else {
          ...
          $("#search_customer").parent().attr("data-refparam", JSON.stringify({
