  ...
  var rendertype = function(viewModel) {
    ...
    return {
      ...
      operation: function(obj) {
        var str = '';
        var billreceiptState = obj.row.value.billreceiptState;
        var state = obj.row.value.state;
        if (billreceiptState == '1' || billreceiptState == '6') {
          // 可编辑,可提交,可删除
          str = operationStr(obj, 'showEditBillPanel', '编辑').concat(operationStr(obj, 'submitCusReqForm', '提交')).concat(operationStr(obj, 'del', '删除'));
        } else if (billreceiptState == '2') {
          // 已提交收款单，尚未审批 . 可收回,可审批
          str = operationStr(obj, 'unsubmitCusReqForm', '收回') + operationStr(obj, 'approveCusReqForm', '审批');
        } else if (billreceiptState == '3') {
          // 已审批收款单 .  目前没操作
          str = operationStr(obj, 'cancelApproveCusReqForm', '取消审批');
        }
