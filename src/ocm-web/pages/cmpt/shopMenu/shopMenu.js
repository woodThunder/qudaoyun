...
define([
    ...
    var view = baseview.extend({
        ...
        model: model,
        baseData: {
            baseurl: '/cmpt/shop-menus',
            statusField: 'isEnable',
            excelurl:'/cmpt/shop-menus-excel',
            dialogWidth: '900px',
            simpleList: new u.DataTable(model.options.metas.ShopMenuMeta),
            simpleListTree: new u.DataTable(model.options.metas.ShopMenuMeta),
            ...
            treeSetting: {
                ...
                callback: {
                    beforeClick: function (treeId, treeData) {
                        var refComp = $("#refContainershopMenuKey").data("uui.refer");
                        if(refComp && treeData) {
        ...
        events: {
            ...
            search: function (reindex) {
                ...
                $._ajax({
                    ...
                    data: queryData,
                    success: function (data) {
                        /*if(queryData.search_LIKE_treeCode ==undefined) {
                            viewModel.searchTree(queryData);
                        }*/
            ...
            clickTree: function () {
                ...
                //如果参数是parent.id的话，这里设置过后会转成treeCode
                var refComp = $("#refContainershopMenuKey").data("uui.refer");
                if(refComp && treeData) {
                    refComp.setValue([treeData]);
                }
