  ...
  OrgChart.prototype = {
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        //为商品结构单独定制
        if(data["type"]=="goodsbom"){
            $nodeDiv.append('<div class="title title-goodsbom">' + data["name"] + '</div>')
            .append( '<div class="content-middle content-goodsbom1"><span>编码:</span>' + (data["code"] || '') + '</div>' )
            .append( '<div class="content-middle content-goodsbom2"><span>单位:</span>' + (data["unit"] || '') + '</div>' )
            .append( '<div class="content-middle content-goodsbom3"><span>数量:</span>' + (data["qty"] || '') + '</div>' )
            .append( '<div class="content-middle content-goodsbom4"><span>规格:</span>' + (data["specification"] || '') + '</div>' )
            .append( '<div class="content-middle content-goodsbom5"><span>型号:</span>' + (data["model"] || '') + '</div>' )
          }else if(data["type"]=="organization-func"){
			if(data['id']){
				var params = {
						id: data['id']
						}
				...
				var hrefValue = "index-view.html?" + urlParam + "#/organization";
			//	var detailfun = "data-bind=click:this.showOrganization.bind($data," + 0 + ")";
				$nodeDiv.append('<div class="title-titleOnly org_'+ data["id"] +'">'+'<a style="color:#fff" href=' + hrefValue + ' value="organization" name="组织" class="ui-a-detail openorg" ' +
					">" + data["name"] + '</a></div>')
			}else{
				$nodeDiv.append('<div class="title-titleOnly org_'+ data["id"] +'">' + data["name"] + '</div>')
			}
          }else{
	  ...
	  $nodeDiv.on('click', '.openorg', { 'nodeData': data }, this.showOrganization.bind(this));
