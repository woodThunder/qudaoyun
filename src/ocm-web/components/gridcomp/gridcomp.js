define([], function () {
	...
	gridCompProto.initDefault = function () {
		...
		this.defaults.rowHeight = 32; // 行高
		this.defaults.headerHeight = 40;
	}
		...
		this.options.afterCreate = function () {
			...
			setTimeout(function () {
				...
				if (!$ele.find(".ps-scrollbar-x").width()) {
					...
					$ele.find(".u-grid-content-div table").css("padding-bottom", "12px")
				}
			}, 100)
		}
	}
	...
	gridCompProto.createThead = function (createFlag) {
		...
		$.each(gridCompColumnArr, function (i) {
			...
			var editableHtml = '';
			if (oThis.options.editable && this.options.editable) {
				editableHtml = '<i class="uifont icon-editable"></i>';
			}
			htmlStr += '<div class="u-grid-header-link" field="' + this.options.field + '" ' + colorStype + '>' + requiredHtml + this.options.title + editableHtml + '</div>';
	...
	u.pagination.prototype.page = function (pageIndex, totalPages, pageSize, oldIndex) {
		...
		if (typeof beforePageChange == "function") {
			...
			oldIndex = oldIndex - 1 < 0 ? 0 : oldIndex - 1;
			var canChange = beforePageChange(tmpPageIndex, totalPages, pageSize, oldIndex);
			if (!canChange) {
				return;
			}
		}
	...
	App.prototype.compsValidateMultiParam = function (options) {
		...
		for (var i = 0; i < comps.length; i++) {
			if (comps[i].doValidate) {
				...
				if (comps[i].type == "u-text" && !(comps[i].validType == "float" || comps[i].validType == "integer")) {
	...
	if (u.GridAdapter) {
        ...
        // 判断元素是否显示
        var eleIfVisible = $('#'+this.grid.options.id + '_content_tbody').is(":visible");
				// 如果元数显示，且存在错误信息，提示信息
				if (!columnPassedFlag && options.showMsg && eleIfVisible) {
			...
			if (columnShowMsg) toastr.warning(columnShowMsg);
			if (MsgArr.length > 0) {
	...
	// 提供新方法，新增后在第一行显示
	u.DataTable.prototype.createEmptyRowF = function (options) {
		var r = new u.Row({
			parent: this
		});
		this.insertRow(0, r);
	...
	u.DataTable.prototype.setSimpleData = function (data, options, usePersistStauts, useRowId) {
		this.removeAllRows();
		this.cachedPages = [];
		this.focusIndex(-1);
		...
		// 排序 sortType="asc"(默认) || "desc"
		var sortType = options && options.sortType ? options.sortType : "asc"
		var rows = [];
		if (!u.isArray(data)) data = [data];
		if (data.length > 1 && data[0].rowNum) {
			data = data.sort(function(a, b){
				return  sortType == "asc" ? a.rowNum - b.rowNum : b.rowNum - a.rowNum
			})
		}
		for (var i = 0; i < data.length; i++) {
			...
			if (usePersistStauts === true && _data.persistStatus) {
				u.extend(temp, {
					status: _data.persistStatus,
				});
			}
			if (useRowId === true && _data.rowId) {
				u.extend(temp, {
					id: _data.rowId,
				});
			}
	...
	u.DataTable.prototype.setExtendValue = function (fieldName, value) {
		if (!fieldName) {
			console.error("setExtendValue的参数不能为空！");
			return;
		}
		if (this.getMeta()[fieldName]) this.setValue(fieldName, value);
	};
	/**
	 *  扩展DateTable方法，setSimpleData$1 新增时是否触发事件
	 *  {boolean} subscribe  是否触发监听，true表示触发监听，默认为true
	 */
	/*u.DataTable.prototype.addSimpleData = function (data, status, options, subscribe) {
	  if (!data) {
	  ...
	  for (var i = 0; i < data.length; i++) {
		  ...
		  r.setSimpleData(data[i], status, subscribe);
	  }
	};*/
    
	// u.Row.prototype.setSimpleData = function setSimpleData(data, status, subscribe) {
	//   var allData = {};
	//   allData.data = data;
	//   allData.status = status || 'nrm';
	//   subscribe = subscribe === false ? subscribe : true;
	//   this.setData(allData, subscribe);
