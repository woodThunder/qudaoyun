...
define(function (require, module, exports) {
    ...
    function loadUpload(callback) {
            ...
            uploader.makeThumb(file, function (error, ret) {
                ...
                if (error) {
                    if (file.ext === 'xls' || file.ext === 'xlsx') {
                        ...
                        $li.text($.i18n.prop('js.use.upl.0001'));
                    }
                } else {
        ...
        uploader.on('uploadProgress', function (file, percentage) {
            ...
            $li.find('span.state').text($.i18n.prop('js.use.upl.0002'));

            $percent.css('width', percentage * 100 + '%');
        });

        uploader.on('uploadSuccess', function (file) {
            $('#' + file.id).find('span.state').text($.i18n.prop('js.use.upl.0003'));
        });

        uploader.on('uploadError', function (file) {
            $('#' + file.id).find('span.state').text($.i18n.prop('js.use.upl.0004'));
        });
