...
/***/ function (module, exports, __webpack_require__) {
			...
			__webpack_require__.e/* nsure */(1, function (sidebar) {
			...
			// window.papTitleInit=function(dom,goBack){
			...
			window.onload = function () {
				...
				if (settings) {
					...
					link.id = "skinStyle";
					head.append($(link));
					if ($.cookie('logoImgUrl')) {
						$('.logo').find('img').attr('src', decodeURIComponent(decodeURIComponent($.cookie('logoImgUrl'))));
					} else {
						if (settings == 'primary') {
							if(window.contextRoot=='/portal'){//企业门户
								$('.logo').find('img').attr('src', 'logo.png');
							}else{//专属云
								$('.logo').find('img').attr('src', 'images/logo.svg');
							}
						} else {
							if(window.contextRoot=='/portal'){
								$('.logo').find('img').attr('src', 'logo_' + settings + '.png');
							}else{
								$('.logo').find('img').attr('src', 'images/logo_' + settings + '.svg');
							}
						}
					}
				...
				$doc.ajaxError(function(event, XMLHttpRequest, ajaxOptions, thrownError){
					if (XMLHttpRequest.status == '306') {
                        window.parent.location.href = window.contextRoot+'/pages/login/login.html?r=L3BvcnRhbC8';
                    }
                    if(XMLHttpRequest.status == '401') {
                        var info =JSON.parse(XMLHttpRequest.responseText).msg;
                        u.messageDialog({msg: info, title: '请求错误', btnText: '确定'});
                    }
                })
				/**
				 * 统一header
				 * @param {dom} dom 加载header的dom元素
				 * @param {string} title 标题
				 * @param {boolean} hideback 是否显示返回按钮，默认true
				 * @param {function} backFn 返回的回掉函数
				 * @param {dom} otherBtn 其它按钮dom
				 */
				window.headerInit = function (dom, title, hideback, backFn, otherBtn) {
					if (typeof backFn != 'function') {
						backFn = function () {
							window.history.go(-1);
						}
					}
					var backstr = '<button class="u-button backbutton" id="papHeader"><i class="uf uf-arrow-left"></i> 返回<span class="u-button-container"><span class="u-ripple"></span></span></button>';
					if (hideback) backstr = '';
					var str = '<div class="pap-title">' + backstr + '<h1>' + title + '</h1>' + (otherBtn ? otherBtn : '') + '</div>';
					dom.innerHTML = str;
					if(!hideback)document.getElementById('papHeader').onclick = backFn;
				}
			};
			//加载更多
			$('#username').on('click', function () {
				//if ($('#username').attr('catch')) return;
						...
						if (res.status == 1) {
							...
							if (res.data && res.data.length) {
								...
								for (var i = 0; i < res.data.length; i++) {
									var temp = res.data[i];
									//数据库配置中 直接iframe进内容区域的urltype为 get ，注销地址的 code 为 logout 。  新窗口打开的urltype为 blank
									if (temp.urlType == 'get') {
										if ($('#logooutUrl').val()) {
											if (temp.code == 'logout') {
												str = '';
											} else {
												str = '<li><a href="' + window.contextRoot + '/' + temp.url + '"><i class="' + temp.icon + '">' +
													'</i>' + temp.name + '</a></li>';
											}
										} else {
											str = '<li><a href="' + window.contextRoot + '/' + temp.url + '"><i class="' + temp.icon + '">' +
									...
									} else if (temp.urlType == 'blank') {//新窗口打开
										jsStr += '$("body #' + temp.code + '").on("click",function(){window.open("' + temp.url + '");});';
									}
								}
								// str='<li role="presentation"><a role="menuitem" href="'+$('#logooutUrl').val()+'"><i aria-hidden="true" class="iconfont  icon-logout"></i> 注销</a></li>';
								if ($('#logooutUrl').val()) {
									str = '<li role="presentation"><a role="menuitem" href="' + $('#logooutUrl').val() + '"><i aria-hidden="true" id="logout" class="iconfont  icon-logout" ></i> 注销</a></li>';
									$('.nav.navbar-nav').find('ul#moreMenu').append(str);
								}
			...
			window.setInterval(function () {
				...
				var exponent = sessionStorage.getItem('exponent');
				if(!exponent){
					var uri = window.contextRoot+'/open/getEncryptParam';
					$.ajax({
						type: 'get',
						dataType: 'json',
						url: uri,
						...
						success: function (res) {
							if (res.status === 1) {
								var exponents = res.data.exponent || '';
								var modulus = res.data.modulus || '';
								sessionStorage.setItem("exponent", exponents);//moded by yany 修改为sessionStorage存取
										...
										var option = {
											...
											success: function (res) {
												if (res.status == '1') {
                                                    ...
                                                    window.message(res.msg||"请求出错",'error');
                                                }
											},
							...
							var option = {
								...
								success: function (res) {
									if (res.status == '1') {
										...
										window.message(res.message||'操作失败', 'error');
									}
							...
							$.ptAjax({
								...
								success: function (res) {
									...
									if (res.data && res.data.content && res.data.content.length) {
										...
										setScrollTop(0);
										$('#content').html(html);
										window.headerInit($('#widgetList .breadcrumb')[0],'小部件管理')
										var reg = /MSIE([\W\w]+?);/;
			...
			var __WEBPACK_AMD_DEFINE_RESULT__; !(__WEBPACK_AMD_DEFINE_RESULT__ = function (require, module, exports) {
				...
				var init = function () {
						...
						$.ptAjax({
							...
							success: function (res) {
								...
								if (res.status == '1') {
									...
									$('#content').html(html);
									window.headerInit($('#layoutList .page-header')[0],'布局管理',false,function(){window.location.hash = '#sysmgr';})
									var reg = /MSIE([\W\w]+?);/;
									var agent = window.navigator.userAgent;
									if (!!window.ActiveXObject || "ActiveXObject" in window) {
										if (reg.exec(agent)) {
					...
					function eventInit() {
						...
						$('#layoutList').on('click', '.layouts-set', function () {
							window.designCode = '';
						});
			...
			!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, module) {
				var init = function (sysCode, content) {
					...
					$('.userRelevance').find('.u-msg-ok').on('click', function () {
						...
						data.push({ id: "pt_modulefrom", value: getParams('modulefrom')||'' });
						...
						$.ptAjax({
							...
							success: function (res) {
								if (res.status == "1") {
									if(window.history.length>1&&(!getParams('modulefrom'))){
										window.message('操作成功，请刷新后重试');
										window.history.go(-1);
									}else{
										window.message('操作成功，3秒后关闭当前页面，请刷新主界面后重试');
										window.setTimeout(function(){
											try{
												window.close();
											}catch(e){
