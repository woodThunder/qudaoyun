define("form/component/billmaker", ["form/component", "form/tplutil", "form/component/userselecter"], function() {
	var m = require("form/component");
	var l = require("form/tplutil"),
		b = require("form/component/userselecter");
	
	window.BillMaker = m.extend({
		initialize: function(a) {
			this.componentSetup = {
				componentKey: "BillMaker",
				title: "制单人",
				...
				size: "large",  //控件尺寸 
				isShowRelateAttr: !1,//是否显示相关属性
				billMakerDept:!1,//相关属性-制单人部门
				billMakerPost:!1,//相关属性-制单人岗位
				billMakerJob:!1,//相关属性-制单人职务
				billMakerDeptLeader:!1,//相关属性-制单人部门负责人
				isReadonly: !1,
				borderColor:"",//边框颜色
				islist:true //是否列表显示
			};
			null != a && (this.componentSetup.title = a.title, 
					...
					this.componentSetup.isShowRelateAttr = a.isShowRelateAttr, 
					this.componentSetup.billMakerDept = a.billMakerDept, 
					this.componentSetup.billMakerPost = a.billMakerPost, 
					this.componentSetup.billMakerJob = a.billMakerJob, 
					this.componentSetup.billMakerDeptLeader = a.billMakerDeptLeader, 
		...
		setIsShowRelateAttr: function(a) {
			this.componentSetup.isShowRelateAttr = a
		},
		setBillMakerDept: function(a) {
			this.componentSetup.billMakerDept = a
		},
		setBillMakerPost: function(a) {
			this.componentSetup.billMakerPost = a
		},
		setBillMakerJob: function(a) {
			this.componentSetup.billMakerJob = a
		},
		setBillMakerDeptLeader: function(a) {
			this.componentSetup.billMakerDeptLeader = a
		},
		...
		render: function(a) {
			var b = $(this.tpl).siblings("#form-billmaker");
		...
		renderEditor: function() {
			var a = $(this.tpl).siblings("#editor-billmaker");
			...
			if("true" == this.componentSetup.isShowRelateAttr || 1 == this.componentSetup.isShowRelateAttr){
				 a.find("#isShowRelateAttr").attr("checked", "true");
				 a.find(".f_relate_attr").removeClass("hide");
			}else{
				a.find(".f_relate_attr").addClass("hide");
			}
			"true" != this.componentSetup.billMakerDept && 1 != this.componentSetup.billMakerDept || a.find("#billmaker_department").attr("checked", "true");
			"true" != this.componentSetup.billMakerPost && 1 != this.componentSetup.billMakerPost || a.find("#billmaker_post").attr("checked", "true");
			"true" != this.componentSetup.billMakerJob && 1 != this.componentSetup.billMakerJob || a.find("#billmaker_job").attr("checked", "true");
			"true" != this.componentSetup.billMakerDeptLeader && 1 != this.componentSetup.billMakerDeptLeader || a.find("#billmaker_depleader").attr("checked", "true");
		...
		renderStatSearch: function(a) {
			var b = this,
				d = $(this.tpl).siblings("#statsearch-billmaker"),
				e = a.parentEl;
			a = a.container;
		...
		getAllActivitis:function(a,b){
			var c = $("#relateProcessDef").is(":checked"),d = "";
			if(!c){
				return;
			}else{
				d = $("#processDefId").val();
