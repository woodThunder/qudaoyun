...
//		});
	...
	//初始化树
	uitemplateCtr.initTree = function(app, viewModel,templateModel,params){
		var treectrl = require("uitree");
		treectrl.initTree(app, viewModel,templateModel,params);
	}
	//树的回调方法
	uitemplateCtr.changeTable = function(app, viewModel,templateModel,treeId,treeNode){
		//todo something
		//console.log(treeId+"---"+treeNode.pk);
		//$("#"+treeNode.tId).toggle(change1(),change2(),true);
		...
		for (var key in dts){
			...
			for (var k in meta){
				...
				if(typeof componentKey !="undefined" && !(componentKey=="Money" || componentKey=="NumberComponent")){
					var mainfieldid = "";
					//var maincr = viewModel.headform.getCurrentRow();
					var maincr = viewModel[key].getCurrentRow();
					if(maincr){
						var val = "666666";
						$('#form-treeObj').hasClass("eventflag") && (val="6890");
